...
3. Add or update claims in `wiki/CLAIMS.md` **only through `gate.py`** — never hand-edit rows. Resolve `WIKI_GATE` through `wiki-update/resolve-gate.py` as specified in the skill contract, build typed-slot JSON, and run `python3 "$WIKI_GATE" --wiki wiki check <candidate.json>`, obeying the verdict (`--apply` on ADMIT). Same gated procedure as `wiki-update`'s `SessionUpdate.md` §7a.
