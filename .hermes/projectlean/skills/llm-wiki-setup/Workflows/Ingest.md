...
9. Add important claims to `wiki/CLAIMS.md` **only through `gate.py`** — never hand-edit claim rows. Resolve `WIKI_GATE` through `wiki-update/resolve-gate.py` as specified in the skill contract, then run `python3 "$WIKI_GATE" --wiki wiki check <candidate.json>` and obey the verdict (`--apply` on ADMIT). This is the same gated procedure as the `wiki-update` skill's `SessionUpdate.md` §7a; follow it rather than writing rows by hand. The gate assigns IDs, enforces the 12-column schema and column rules, and bounds the budget. Candidate claims must point to `wiki/candidates/...` until promotion.
10. Update indexes: add candidates to the root `wiki/index.md` candidate review queue; add promoted pages to their directory's `index.md` (OKF bullet listing). Keep the root index's directory enumeration current.
...
- Root `index.md` lists candidates in the candidate review queue without treating them as promoted pages.
