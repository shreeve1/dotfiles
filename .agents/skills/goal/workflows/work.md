...
- **Stopping condition appears met** → DO NOT set `Status: done` directly.
  - **If `GOAL.md` has `Validation is read-only: no`** → the verifier is disabled for this goal (it would re-run a mutating validation command). Skip the verify workflow. Instead, surface the evidence to the user (the passing validation result and what it proves) and ask for explicit confirmation that the stopping condition is met. Only set `Status: done` after the user confirms; append a `VERIFY` entry with `Verdict: user-confirmed (verifier disabled — validation not read-only)` before flipping status. If the user does not confirm, stay `active` or set `Status: blocked` per their direction.
  - **Otherwise (`Validation is read-only: yes`)** → run the **verify** workflow ([verify.md](verify.md)). The verifier spawns a fresh Claude Code session, re-runs validation independently, and returns a structured verdict in `<goal_dir>/.verify-last.json`. Handle the outcome in this order:
