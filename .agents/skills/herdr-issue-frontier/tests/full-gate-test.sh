...
[[ -x "$SCRIPT" ]] || {
	echo "FAIL: full-gate.sh not found at $SCRIPT" >&2
	exit 1
}
bash -n "$SCRIPT" || {
	echo "FAIL: bash -n full-gate.sh" >&2
	exit 1
}
...
pass=0
fail=0
ck() { if eval "$1"; then pass=$((pass + 1)); else
	fail=$((fail + 1))
	echo "FAIL: $2" >&2
fi; }

# 1. env override runs the command and exits 0.
o=$(HERDR_FRONTIER_TEST_CMD="echo GATEOK" bash "$SCRIPT" 2>&1)
rc=$?
...
HERDR_FRONTIER_TEST_CMD="exit 7" bash "$SCRIPT" >/dev/null 2>&1
rc=$?
ck "(( rc == 7 ))" "failing gate should propagate non-zero exit"

# 3. opt-out: skipped, exit 0.
o=$(HERDR_FRONTIER_NO_GATE=1 bash "$SCRIPT" 2>&1)
rc=$?
