...
- [Phase 2: RESOLVE AMBIGUITY](#phase-2-resolve-ambiguity)
- [Phase 3: LOCATE](#phase-3-locate)
- [Phase 4: VERIFY LOCATION](#phase-4-verify-location)
- [Phase 5: CONFIRM ROOT CAUSE](#phase-5-confirm-root-cause)
- [Phase 6: STOP AT DIAGNOSIS](#phase-6-stop-at-diagnosis)
...
- [Write Fix Tests](#write-fix-tests)
  ...
  - [Create the directory](#create-the-directory)
  - [Write `tests/regression/fix-$TIMESTAMP.<ext>`](#write-testsregressionfix-timestampext)
  - [Untestable root causes](#untestable-root-causes)
  - [Confirm red/green status](#confirm-redgreen-status)
...
Use `ask the user` to confirm understanding and resolve ambiguity:
...
Run a focused code search. Use a subagent only if the user explicitly asked for delegated investigation and the current session supports it.

```
Use targeted code search to find:
...
The search pass should:
1. Search for keywords from error messages
2. Find relevant files using find files/search
3. Report structured findings with file:line references

**If deeper investigation is needed after the initial search:**
