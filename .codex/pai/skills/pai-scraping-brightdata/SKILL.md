---
name: "pai-scraping-brightdata"
description: "PAI Codex port: 4-tier progressive URL scraping and multi-page crawling - web access, then Chrome-header curl, then Playwright browser, then Bright Data proxy. Auto-escalates when lower tiers fail. USE WHEN Bright Data, scrape URL, web..."
---
...
`.codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/BrightData/`
...
- Standard methods (web access) are failing
...
1. **Tier 1: web access** - Fast, simple, built-in Codex tool
...
- **Process:** Start with web access -> If fails, use curl with Chrome headers -> If fails, use Browser Automation -> If fails, use Bright Data MCP
...
**Integration Points:**
- **web access Tool** - Built-in Codex tool for basic URL fetching
...
- **Tier 1 (web access):** Simple sites, public content, no bot detection
...
**Configuration:**
No configuration required - all tools are available by default in Codex
...
2. Attempts Tier 1 (web access) -> Fails (blocked)
...
- `.codex/pai/PAI/SkillSystem.md` - Canonical structure guide
- `.codex/pai/PAI/CONSTITUTION.md` - Overall PAI philosophy
