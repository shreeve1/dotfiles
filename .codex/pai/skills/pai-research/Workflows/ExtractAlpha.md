...
Based on Codex Shannon's information theory: **real information is what's different, not what's the same.**
...
```bash
.codex/pai/MEMORY/WORK/{current_work}/
```

**To get the current work directory:**
1. Read `.codex/pai/MEMORY/STATE/current-work.json`
2. Extract the `work_dir` value
3. Use `.codex/pai/MEMORY/WORK/{work_dir}/` for temporary artifacts
...
```
.codex/pai/MEMORY/WORK/20260111-172408_extract-alpha-analysis/
 raw-transcript.txt
...
```bash
.codex/pai/History/research/YYYY-MM-DD_description/
```
...
```
.codex/pai/History/research/2025-10-26_podcast-analysis/
...
**usually verify output is properly captured:**

1. **Check if hooks captured the output:**
   ```bash
   # Check most recent history entries
   ls -lt .codex/pai/History/research/ | head -5

   # Verify your research directory exists
   ls .codex/pai/History/research/YYYY-MM-DD_description/
   ```

2. **If hooks did NOT capture automatically:**
   ```bash
   # Create directory structure manually
   mkdir -p .codex/pai/History/research/YYYY-MM-DD_description/
...
3. **Confirm all files saved:**
   ```bash
   ls -lah .codex/pai/History/research/YYYY-MM-DD_description/
...
WORK_DIR=$(jq -r '.work_dir' .codex/pai/MEMORY/STATE/current-work.json)

# 2. Work in current work item directory
cd .codex/pai/MEMORY/WORK/${WORK_DIR}/
...
mkdir -p .codex/pai/History/research/$(date +%Y-%m-%d)_podcast-analysis/
...
ls -lah .codex/pai/History/research/$(date +%Y-%m-%d)_podcast-analysis/
...
// PAI:
// 1. Fetches content (web access or direct paste)
...
1. **Load content** via appropriate method (fabric -y, web access, Read, or paste)
2. **Get current work directory** - Read `.codex/pai/MEMORY/STATE/current-work.json` for `work_dir`
3. **Use work item directory** - Work in `.codex/pai/MEMORY/WORK/{work_dir}/`
...
6. **Save to history** - Final outputs to `.codex/pai/History/research/YYYY-MM-DD_description/`
...
**Four-step process:**
1. Extract content (fabric -y, web access, Read, paste)
