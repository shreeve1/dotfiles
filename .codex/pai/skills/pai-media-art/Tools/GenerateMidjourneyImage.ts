 ...
 *
 * @see .codex/pai/skills/art/SKILL.md
 */
...
/**
 * Load environment variables from CODEX_PAI_ROOT/.env
...
async function main() {
  try {
    // Load API keys from CODEX_PAI_ROOT/.env
    await loadEnv();
    ...
    if (!botToken) {
      throw new CLIError(
        'Missing DISCORD_BOT_TOKEN environment variable. Add it to CODEX_PAI_ROOT/.env'
      );
    }

    if (!channelId) {
      throw new CLIError(
        'Missing MIDJOURNEY_CHANNEL_ID environment variable. Add it to CODEX_PAI_ROOT/.env'
      );
    }
