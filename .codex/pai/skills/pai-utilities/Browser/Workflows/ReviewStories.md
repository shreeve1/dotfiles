...
For each individual story, spawn one UIReviewer agent via the Codex subagent tools. **All spawn_agent calls go in a single message** for true parallelism.
...
- **spawn_agent parallelism, not TeamCreate.** UIReviewers are stateless parallel workers. Multiple `spawn_agent(subagent_type="UIReviewer")` calls in one message achieve true parallelism without swarm coordination overhead.
