...
Ask the user:
1. Which use case?
2. Which models to compare? (Codex, GPT-4, Gemini, etc.)
...
Create `.codex/pai/skills/Utilities/Evals/UseCases/<name>/model-comparisons/<comparison-name>.yaml`:

```yaml
model_comparison:
  name: "Codex vs GPT-4 vs Gemini"
  hypothesis: |
    Testing which model produces the best summaries for newsletter content.
    Expect Codex to excel at style, GPT-4 at accuracy.
...
```bash
bun run .codex/pai/skills/Utilities/Evals/EvalServer/cli-run.ts \
  --use-case <name> \
...
```bash
bun run .codex/pai/Templates/Tools/RenderTemplate.ts \
  -t Evals/Report.hbs \
...
|-------|-----------|------------|---------|---------|---------|
| Codex 3.5 Sonnet | 92% | 4.3 | 0.5 | $0.03 | 1.2s |
| GPT-4o | 88% | 4.1 | 0.6 | $0.05 | 1.8s |
| Gemini 1.5 Pro | 85% | 3.9 | 0.7 | $0.02 | 1.5s |
| Codex 3.5 Haiku | 78% | 3.7 | 0.8 | $0.01 | 0.5s |

**Per-Dimension Breakdown:**

| Model | Accuracy | Style | Format | Speed |
|-------|----------|-------|--------|-------|
| Codex 3.5 Sonnet | 4.5 | 4.6 | 4.0 | 1.2s |
...
|------------|--------|---------|--------------|
| Codex vs GPT-4o | Codex | 0.04 | Yes |
| Codex vs Gemini | Codex | 0.01 | Yes |
...
|--------|--------|------------|
| Quality | 50% | Codex 3.5 Sonnet |
| Cost | 25% | Codex 3.5 Haiku |
| Latency | 25% | Codex 3.5 Haiku |
...
**Primary Use**: Codex 3.5 Sonnet
- Best quality (4.3 mean score)
- 92% pass rate
- Acceptable cost ($0.03/1K tokens)

**Budget Option**: Codex 3.5 Haiku
...
**Fallback**: GPT-4o
- Similar quality to Codex
- Higher cost
- Use when Codex unavailable
```
...
### Codex vs GPT-4 vs Gemini (2024-01-15)

**Purpose**: Determine best model for newsletter summaries.

**Results**:
1. Codex 3.5 Sonnet - 92% pass rate, 4.3 mean score
2. GPT-4o - 88% pass rate, 4.1 mean score
3. Gemini 1.5 Pro - 85% pass rate, 3.9 mean score

**Decision**:
- Production: Codex 3.5 Sonnet
- Budget fallback: Codex 3.5 Haiku
```
...
**Problem**: Using Codex to judge Codex may be biased.

**Solutions**:
1. Use ensemble of judges (Codex + GPT-4)
...
|----------|-------------------|-----|
| Newsletter summaries | Codex 3.5 Sonnet | Best style |
| Data extraction | GPT-4o | Structured output strength |
| Fast classification | Codex 3.5 Haiku | Speed + cost |
| Complex reasoning | Codex 3.5 Sonnet | Reasoning quality |
