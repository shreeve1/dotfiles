...
import type { AlgorithmEvalRequest, AlgorithmEvalResult, EvalRun, spawn_agent } from '../Types/index.ts';
      ...
      executor: async (t, trialNum) => {
        ...
        const transcript = createTranscript(t.id, `trial_${trialNum}`, {
          turns: [
            ...
            { role: 'assistant', content: 'spawn_agent executed via ALGORITHM' },
          ],
          toolCalls: [],
        });
...
export async function updateISCWithResult(result: AlgorithmEvalResult): Promise<void> {
  ...
  await $`bun run .codex/pai/skills/THEALGORITHM/Tools/ISCManager.ts update --row ${result.isc_row} --status ${status} --note "${formatForISC(result)}"`.quiet();
}
