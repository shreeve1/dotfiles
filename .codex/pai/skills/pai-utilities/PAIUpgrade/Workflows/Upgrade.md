...
```
Use Codex subagent tools with subagent_type=general-purpose, run 4 agents in parallel:

Agent 1 - TELOS Analysis:
"Read and analyze the user's TELOS files to understand their current focus:
- .codex/pai/PAI/USER/TELOS/TELOS.md
- .codex/pai/PAI/USER/TELOS/GOALS.md
- .codex/pai/PAI/USER/TELOS/PROJECTS.md
- .codex/pai/PAI/USER/TELOS/CHALLENGES.md
- .codex/pai/PAI/USER/TELOS/STATUS.md
...
- Read .codex/pai/MEMORY/STATE/current-work.json
...
- List skills in .codex/pai/skills/
- List hooks in .codex/pai/hooks/
- Read .codex/pai/settings.json
...
```
Use Codex subagent tools with subagent_type=general-purpose, run 3 agents in parallel:
...
1. Load channel config:
   bun .codex/pai/PAI/Tools/LoadSkillConfig.ts ../youtube-channels.json
...
4. For NEW videos, extract transcripts:
   bun .codex/pai/PAI/Tools/GetTranscript.ts '<video-url>'
...
DO NOT return vague summaries like 'discusses Codex features'.
...
1. Look in .codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/
...
1. Load the github_trending config from user-sources.json:
   Read .codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/user-sources.json
...
```
Use Codex subagent tools with subagent_type=general-purpose, run 1 agent in parallel with above:
...
5. **FOUR TIERS, when appropriate** - Every technique maps to exactly one of:  CRITICAL,  HIGH,  MEDIUM,  LOW. Empty tiers are fine - show them anyway.
...
9. **TWO important DESCRIPTIONS (16-32 words each):**
...
| # | Discovery | Source | Why It's Interesting | PAI Relevance |
|---|-----------|--------|---------------------|---------------|
| 1 | PreToolUse hooks can inject reasoning context | claude-code v2.1.16 | Hooks can now return `additionalContext` that Codex reasons about before tool execution - this is a paradigm shift from binary block/allow to intelligent security | SecurityValidator could inject warnings instead of blocking, enabling context-aware security decisions |
...
**How It Helps PAI (16-32 words):**
SecurityValidator.hook.ts currently blocks dangerous commands. With additionalContext, it can inject warnings Codex reasons about, enabling smarter security that adapts to context.
...
| Gemini 3 videos | YouTube | Not relevant to Codex-centric stack |
