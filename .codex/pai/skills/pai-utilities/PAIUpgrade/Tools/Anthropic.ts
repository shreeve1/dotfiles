...
function generateRecommendation(update: Update): string {
  ...
  if (titleLower.includes('command') || titleLower.includes('slash command')) {
    return `**PAI Impact:** HIGH - Command system update\n` +
      `**Why:** PAI uses slash commands extensively (.codex/pai/Commands/). Changes affect our command architecture and user workflows.\n` +
  ...
  if (type === 'release' && source.includes('claude-code')) {
    return `**PAI Impact:** CRITICAL - Core platform update\n` +
      `**Why:** PAI runs on Codex - releases may include new features, breaking changes, or performance improvements.\n` +
  ...
  if (source.includes('cookbook') || source.includes('quickstart') || source.includes('courses')) {
    return `**PAI Impact:** MEDIUM - Implementation patterns\n` +
      ...
      `**Action:** Review for reusable patterns, especially around skills, agents, or Codex features. Extract learnings for PAI.`;
  }
  ...
  if (titleLower.includes('doc') || type === 'docs') {
    return `**PAI Impact:** MEDIUM - Capability discovery\n` +
      ...
      `**Action:** Review for new Codex features, API capabilities, or configuration options to leverage.`;
  }

  // SDK releases
  if (source.includes('sdk')) {
    return `**PAI Impact:** LOW - SDK update\n` +
      `**Why:** SDK updates are less relevant since PAI uses Codex CLI, not raw API SDKs.\n` +
  ...
  if (type === 'blog') {
    return `**PAI Impact:** LOW-MEDIUM - Awareness\n` +
      ...
      `**Action:** Skim for strategic announcements about Codex, Skills, or MCP. Track for future planning.`;
  }
...
function generateNarrative(updates: Update[]): string {
  ...
  if (codeUpdates.length > 0) {
    themes.push(`**Codex Platform** (${codeUpdates.length} updates)`);
  }
  ...
  if (codeUpdates.length > 0) {
    ...
    if (highCode > 0) {
      narrative += `** PLATFORM UPDATE: Codex Changes**\n`;
      narrative += `**${highCode} HIGH-priority** Codex updates found. Since PAI runs on Codex, platform changes can affect everything. `;
      ...
      narrative += `Codex has **${codeUpdates.length} updates** but lower priority. Likely maintenance commits.\n\n`;
    }
  }
  ...
  if (codeUpdates.filter(u => u.priority === 'HIGH').length > 0) {
    topItems.push('3. **Codex platform** - Review release notes, test workflows');
  }
