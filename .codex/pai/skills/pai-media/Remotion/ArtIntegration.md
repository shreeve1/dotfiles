...
**important:** This skill inherits visual theming from the Art skill.

## Before Creating Any Video Content

1. **Load Art preferences:**
   ```
   .codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/Art/PREFERENCES.md
   ```
...
3. **Use Theme Constants:**
   ```
   .codex/pai/skills/Remotion/Tools/Theme.ts
   ```

4. **Reference images** (when visual style reference needed):
   ```
   .codex/pai/skills/Media/Art/Examples/
   ```

## PAI Theme Quick Reference

```typescript
import { PAI_THEME } from '.codex/pai/skills/Remotion/Tools/Theme'
...
```typescript
import { PAI_THEME, titleScreenStyle, fadeInterpolation } from '.codex/pai/skills/Remotion/Tools/Theme'
...
**All videos should use this theme unless explicitly overridden.**
