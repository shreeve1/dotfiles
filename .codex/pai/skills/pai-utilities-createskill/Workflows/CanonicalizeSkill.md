...
```bash
.codex/pai/skills/[skill-name]/SKILL.md
```
...
```bash
cp -r .codex/pai/skills/[skill-name]/ .codex/pai/History/Backups/[skill-name]-backup-$(date +%Y%m%d)/
```

**Note:** Backups go to `.codex/pai/History/Backups/`, avoid inside skill directories.
...
cd .codex/pai/skills/[SkillName]/Workflows/
...
find .codex/pai/skills/[SkillName]/ -type d -mindepth 2 -maxdepth 3
```
