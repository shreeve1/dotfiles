...
SCAN_FILE=.codex/pai/MEMORY/WORK/${WORK_DIR}/$(date +%Y-%m-%d)_nmap-scan.txt

# Or masscan results
MASSCAN_FILE=.codex/pai/MEMORY/WORK/${WORK_DIR}/$(date +%Y-%m-%d)_masscan.json

# Or combined multi-tool archive from history
SCAN_DIR=.codex/pai/History/security/scans/2025-11-target-recon/
```
...
// Usage: bun run tools/batch-scan-analysis.ts .codex/pai/MEMORY/WORK/{current_work}/target-scans/
```
...
# Fallback to Codex Sonnet with extended thinking
