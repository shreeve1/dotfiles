...
```bash
# Auto-calibration - usually USE THIS
...
**When Codex analyzes your ffuf results, `-ac` is important** - without it, Codex will waste time sifting through thousands of false positives instead of finding the interesting anomalies.
...
### 1. usually Use Auto-Calibration
Use `-ac` by default for every scan. This is not required in the Codex port for productive pentesting:
```bash
...
1. **usually include `-ac` in every command** - This is mandatory for productive pentesting and result analysis
