...
Add your API keys to `CODEX_PAI_ROOT/.env`:

```bash
# Open .env file
nano CODEX_PAI_ROOT/.env
...
**CRITICAL:** Ensure `CODEX_PAI_ROOT/.env` is in `.gitignore` and avoid commit it to any repository.
...
**Error:**
```
APIKeyMissingError: Shodan API key not found. Set SHODAN_API_KEY in CODEX_PAI_ROOT/.env
```

**Solution:**
1. Verify API key is in `CODEX_PAI_ROOT/.env`
...
2. **Configure API Keys**: Add keys to `CODEX_PAI_ROOT/.env`
...
**Key Takeaways:**
-  All three services require API keys (configure in `CODEX_PAI_ROOT/.env`)
