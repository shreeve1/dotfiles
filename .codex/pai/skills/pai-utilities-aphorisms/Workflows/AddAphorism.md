...
- Use web search to verify wording and attribution
...
web search("\"[quote text]\" [author name] quote")

# Verify attribution
web search("[author name] quotes + [key words from quote]")

# Check common misattributions
web search("misattributed quotes [author name]")
```
...
**Read database:**
```bash
Read .codex/pai/skills/aphorisms/Database/aphorisms.md
```
...
```bash
web search("[author name] '[quote snippet]' context source")
web search("[author name] biography + [time period/work]")
```
...
Edit(
  file_path=.codex/pai/skills/aphorisms/Database/aphorisms.md,
...
### Step 2: Verify
```bash
web search("\"The important thing is not to stop questioning\" Einstein")
```
