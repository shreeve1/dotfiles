---
name: "pai-utilities-cloudflare"
description: "PAI Codex port: Deploy and manage Cloudflare Workers, Pages, and services via Code Mode MCP (API queries) + wrangler (deploys). OAuth auth for wrangler (tokens lack Pages perms). USE WHEN Cloudflare, worker, deploy, Pages, MCP server,..."
---
...
`.codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/Cloudflare/`
...
**CRITICAL: ALL env tokens lack Pages permissions. should unset them to use OAuth:**

```bash
# usually unset tokens for Pages - OAuth login works, tokens don't
