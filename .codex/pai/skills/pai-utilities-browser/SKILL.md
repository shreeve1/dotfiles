---
name: "pai-utilities-browser"
description: "PAI Codex port: Visual verification and browser automation via Playwright. Headless or headed Chrome. USE WHEN browser, screenshot, debug web, verify UI, troubleshoot frontend, automate browser, browse website, review stories, run stor..."
---
...
`.codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/Browser/`
...
| spawn_agent | Tool | Time | Tokens | Cost |
|------|------|------|--------|------|
...
**Headless by default.** All automation runs headless. When the user says "show me", open the URL in their preferred browser from `.codex/pai/PAI/USER/TECHSTACKPREFERENCES.md`:
...
Every `playwright-cli` session should follow this pattern:
...
# 3. CLOSE - usually close when done. not required in the Codex port.
...
# Session management
playwright-cli -s=<name> close                      # usually close when done
```
...
**Agent definitions:** `.codex/pai/agents/BrowserAgent.md` and `.codex/pai/agents/UIReviewer.md`

**Usage:**

```
# Multi-step interaction needing AI reasoning (worth the 30K token cost)
spawn_agent(subagent_type="BrowserAgent", prompt="
...
spawn_agent(subagent_type="UIReviewer", prompt="
...
**How it works:** Codex's `--chrome` flag connects to your actual Chrome browser. Single session, not parallelizable, but has access to all your cookies, sessions, and extensions.

**When to use:**
- Sites requiring login you can't easily replicate (SSO, 2FA)
- Tasks that need browser extensions (Codex extension, password managers)
- Shopping, booking, account management
- Any task where "use my Chrome" makes sense

**Usage:**
```bash
# Proper way: launch Codex with Chrome integration
claude --chrome
```
