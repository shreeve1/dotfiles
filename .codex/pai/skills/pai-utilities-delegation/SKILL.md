---
name: "pai-utilities-delegation"
description: "PAI Codex port: Parallelize work via background/foreground agents, built-in types, custom agents, or agent teams/swarms. USE WHEN 3+ independent workstreams, parallel execution, agent specialization, Extended+ effort, agent team, swarm..."
---
...
Use `spawn_agent(subagent_type="AgentType")` with these specialized agents:
...
```
spawn_agent(subagent_type="Engineer", isolation: "worktree", prompt="...")
```
...
```
spawn_agent(subagent_type="Engineer", run_in_background: true, prompt="...")
```
...
Standard `spawn_agent()` calls that block until complete:
...
**Action:** Invoke the **Agents skill** -> run `ComposeAgent.ts` -> launch with `spawn_agent(subagent_type="general-purpose")`

```bash
# Step 1: Compose agent identity
bun run .codex/pai/skills/Agents/Tools/ComposeAgent.ts --traits "security,skeptical,thorough" --task "Review auth" --output json

# Step 2: Launch with composed prompt
spawn_agent(subagent_type="general-purpose", prompt=<ComposeAgent JSON .prompt field>)
```
...
**Action:** Use `TeamCreate` tool -> `TaskCreate` -> spawn teammates via `spawn_agent(team_name=...)` -> coordinate via `SendMessage`

```
1. TeamCreate(team_name="my-project")           # Creates team + task list
2. TaskCreate(subject="Implement auth module")   # Create team tasks
3. spawn_agent(subagent_type="Engineer", team_name="my-project", name="auth-engineer")  # Spawn teammate
...
### 6. Parallel spawn_agent Dispatch

For N identical operations (e.g., updating 10 files with the same pattern):

1. Create N `spawn_agent()` calls in a single message (parallel launch)
...
```
spawn_agent(subagent_type="general-purpose", model="haiku", max_turns=3, prompt="...")
```
...
```
spawn_agent(subagent_type="general-purpose", prompt="...")  # or specialized agent type
```
