...
**ALL text should be inside `<p>`, `<h1>`-`<h6>`, `<ul>`, or `<ol>` tags:**
-  Correct: `<div><p>Text here</p></div>`
-  Wrong: `<div>Text here</div>` - **Text will NOT appear in PowerPoint**
-  Wrong: `<span>Text</span>` - **Text will NOT appear in PowerPoint**
- Text in `<div>` or `<span>` without a text tag will be silently ignored

**avoid use manual bullet symbols (-, -, *, etc.)** - Use `<ul>` or `<ol>` lists instead
...
- **usually create gradient/icon PNGs FIRST using Sharp, then reference in HTML**
...
#### Colors
- **avoid use `#` prefix** with hex colors in PptxGenJS - causes file corruption
