...
function loadConfig(): Config {
  ...
  try {
    ...
    if (!apiKey) {
      console.error('Error: API_KEY not found in CODEX_PAI_ROOT/.env');
    ...
    console.error('Error: Cannot read CODEX_PAI_ROOT/.env');
    console.error('Create file: touch CODEX_PAI_ROOT/.env');
    process.exit(1);
  }
}
```
...
**Key principles:**
- Load from CODEX_PAI_ROOT/.env (PAI standard)
...
CONFIGURATION:
  API Key: CODEX_PAI_ROOT/.env (API_KEY=your_key)
...
For full documentation: .codex/pai/Bin/${CLI_NAME}/README.md
Version: ${VERSION}
`);
}
```
...
- [ ] Configuration loading (from CODEX_PAI_ROOT/.env)
