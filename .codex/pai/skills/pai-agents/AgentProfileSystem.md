...
## spawn_agent-Specific Knowledge
...
When you need to spawn an agent, use the Codex subagent tools with the agent's context:
...
2. **Agent-specific context** (from `*Context.md` file)
   ...
   - spawn_agent-specific knowledge pointers
...
bun run .codex/pai/skills/Agents/Tools/LoadAgentContext.ts

# View context for specific agent
bun run .codex/pai/skills/Agents/Tools/LoadAgentContext.ts Architect

# Generate enriched prompt for spawning
bun run .codex/pai/skills/Agents/Tools/LoadAgentContext.ts Architect "Design new skill system"
```
...
1. Create `[AgentType]Context.md` in `.codex/pai/skills/Agents/`
...
// Spawn with Codex subagent tools
await spawn_agent({
  subagent_type: "general-purpose",
...
3. **spawn_agent History**: Track which agents worked on which tasks for continuity
