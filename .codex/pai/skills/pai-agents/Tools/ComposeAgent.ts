 ...
 *
 * Configuration files:
 *   Base:  .codex/pai/skills/Agents/Data/Traits.yaml
 *   User:  .codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
 *
 * Usage:
...
/**
 * Save a composed agent to .codex/pai/custom-agents/{slug}.md
 *
 * Produces a CLAUDE CODE COMPATIBLE agent file that can be:
 * 1. Copied to .codex/pai/agents/ and used as a built-in agent
...
/**
 * Build a Codex compatible agent body (system prompt).
 *
 * Matches the structural format of built-in agents in .codex/pai/agents/*.md:
...
#  important STARTUP SEQUENCE - DO THIS FIRST

**BEFORE ANY WORK, YOU should:**
...
**This is not required in the Codex port. Announce yourself first.**
...
**CRITICAL:**
- STORY EXPLANATION should BE A NUMBERED LIST (1-8 items)
...
\`\`\`bash
bun run .codex/pai/skills/Agents/Tools/ComposeAgent.ts --load "${slug}"
\`\`\`

Or reconstruct from traits:

\`\`\`bash
bun run .codex/pai/skills/Agents/Tools/ComposeAgent.ts --traits "${agent.traits.join(",")}"
\`\`\`
...
OPTIONS:
  -t, --task <desc>    spawn_agent description (traits will be inferred)
  ...
  -s, --save           Save composed agent to .codex/pai/custom-agents/
...
CONFIGURATION:
  Base traits:    .codex/pai/skills/Agents/Data/Traits.yaml
  User traits:    .codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
  Custom agents:  .codex/pai/custom-agents/
