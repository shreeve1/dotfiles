...
- `LEARNING/ALGORITHM/YYYY-MM/` - spawn_agent execution learnings (approach errors)
...
| `ALGORITHM/` | spawn_agent execution issues | wrong approach, over-engineered, missed the point |
...
Codex -> projects/{uuid}.jsonl (native transcript)
...
### Check current work
```bash
cat .codex/pai/MEMORY/STATE/current-work.json
ls .codex/pai/MEMORY/WORK/ | tail -5
```

### Check ratings
```bash
tail .codex/pai/MEMORY/LEARNING/SIGNALS/ratings.jsonl
```
...
ls -lt .codex/pai/projects/-Users-{username}--claude/*.jsonl | head -5

# View last session events
tail .codex/pai/projects/-Users-{username}--claude/$(ls -t .codex/pai/projects/-Users-{username}--claude/*.jsonl | head -1) | jq .
```

### Check learnings
```bash
ls .codex/pai/MEMORY/LEARNING/SYSTEM/
ls .codex/pai/MEMORY/LEARNING/ALGORITHM/
ls .codex/pai/MEMORY/LEARNING/SYNTHESIS/
```

### Check failures
```bash
# List recent failure captures
ls -lt .codex/pai/MEMORY/LEARNING/FAILURES/$(date +%Y-%m)/ 2>/dev/null | head -10

# View a specific failure
cat .codex/pai/MEMORY/LEARNING/FAILURES/2026-01/*/CONTEXT.md | head -100

# Migrate historical low ratings to FAILURES
bun run .codex/pai/PAI/Tools/FailureCapture.ts --migrate
```

### Check multi-session progress
```bash
ls .codex/pai/MEMORY/STATE/progress/
```

### Run harvesting tools
```bash
# Harvest learnings from recent sessions
bun run .codex/pai/PAI/Tools/SessionHarvester.ts --recent 10

# Generate pattern synthesis
bun run .codex/pai/PAI/Tools/LearningPatternSynthesis.ts --week
```
...
- Eliminated RAW/ directory entirely - Codex's `projects/` is the source of truth
...
- Removed archive functionality from pai.ts (Codex handles 30-day cleanup)
...
- Moved BACKUPS to `.codex/pai/BACKUPS/` (outside MEMORY)
- Renamed RAW-OUTPUTS to RAW
- All directories now ALL CAPS

**2026-01-05:** v1.0 - Unified Memory System migration
- Previous: `.codex/pai/history/`, `.codex/pai/context/`, `.codex/pai/progress/`
- Current: `.codex/pai/MEMORY/`
