...
|--------|--------|---------|---------|
| `AGENTS.md` | `AGENTS.md.template` + `settings.json` + `PAI/Algorithm/LATEST` | `bun PAI/Tools/BuildCLAUDE.ts` | SessionStart hook + manual |

## Extending PAI

- **Add a skill:** Use the CreateSkill skill under Utilities
- **Add a hook:** Create handler in `.codex/pai/hooks/handlers/`, register in `settings.json`
