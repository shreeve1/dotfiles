...
**The important configuration system for ALL PAI skills.**
...
**ALL skill creation should follow this structure** - including skills created by the CreateSkill skill.
...
## TitleCase Naming Convention (important)

**All naming in the skill system should use TitleCase (PascalCase).**
...
- Reference `.codex/pai/PAI/USER/` for any personalization
...
- Will avoid be pushed to public PAI
...
- `.codex/pai/PAI/USER/CONTACTS.md` - Contact information
- `.codex/pai/PAI/USER/TECHSTACKPREFERENCES.md` - Tech preferences
```

**avoid hardcode personal data in system skills.**
...
`.codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/{SkillName}/`
...
```
.codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/
...
1. **Create directory**: `mkdir -p .codex/pai/PAI/USER/SKILLCUSTOMIZATIONS/SkillName`
...
- `USE WHEN` keyword is **important** (Codex parses this for skill activation)
...
**See:** `.codex/pai/skills/Science/Protocol.md` for the full protocol interface
...
**avoid create a Context/ or Docs/ subdirectory.**
...
skills/SkillName/
 SKILL.md
 Context/               avoid CREATE THIS DIRECTORY
     Aesthetic.md
     Examples.md
```
...
Read .codex/pai/skills/Media/Art/Aesthetic.md
```
...
```
.codex/pai/skills/Createskill/Workflows/CanonicalizeSkill.md
```
...
5. Move backups to `.codex/pai/MEMORY/Backups/`
...
**Every skill should have an `## Examples` section** showing 2-3 concrete usage patterns.

**Why Examples Matter:**
- Anthropic research shows examples improve tool selection accuracy from 72% to 90%
- Descriptions tell Codex WHEN to activate; examples show HOW the skill works
- Codex learns the full input->behavior->output pattern, not just trigger keywords
...
**Reference:** `.codex/pai/skills/_PERSONAL/_MYSKILL/SKILL.md`
...
SkillName/                    # TitleCase directory name
 ...
 Tools/                    # CLI tools (usually present, even if empty)
...
- **Tools/** - CLI tools for automation (usually present directory, even if empty)
...
## Flat Folder Structure (important)
...
**See:** `.codex/pai/PAI/CLIFIRSTARCHITECTURE.md` (Workflow-to-Tool Integration section)
...
**Every skill should have a `tools/` directory**, even if empty. CLI tools automate repetitive tasks and manage stateful resources.
...
**See:** `.codex/pai/PAI/CLIFIRSTARCHITECTURE.md` (Configuration Flags section) for full documentation
 ...
 *
 * Usage:
 *   bun .codex/pai/skills/SkillName/Tools/ToolName.ts <command> [options]
 *
 * Commands:
...
1. **Skill Activation**: Codex reads skill descriptions at startup. The `USE WHEN` clause in the description determines when the skill activates based on user intent.
