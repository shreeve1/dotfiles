...
function writePortingDoc(sourceRoot: string, summary: GeneratedSummary): void {
  ...
  writeText(
    ...
    [
      ...
      "- `session-capture.ts` blocks `Stop` finalization for the active substantive session when the final assistant message lacks verification, review, or learning signals. Re-entrant Stop events and unrelated sessions are allowed to avoid loops.",
      ...
      "",
      "Algorithm enforcement is a context-injection and state-tracking layer. It does not run a separate autonomous agent, and it does not block every edit before a PRD exists. It can block finalization of active substantive work when the final response has not reviewed the work, reported verification, or considered learning. This keeps Codex usable for small fixes while making non-trivial work visibly accountable to the PAI loop.",
      "",
