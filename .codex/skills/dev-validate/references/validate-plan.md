...
- [Phase 1: Plan Selection](#phase-1-plan-selection)
- [Phase 1.5: Understanding Verification](#phase-15-understanding-verification)
- [Phase 1.6: Feasibility Preflight](#phase-16-feasibility-preflight)
- [Phase 1.75: Baseline Provenance Preflight](#phase-175-baseline-provenance-preflight)
- [Phase 2: Smart Validation Analysis](#phase-2-smart-validation-analysis)
- [Phase 3: Targeted Validation](#phase-3-targeted-validation)
  - [Breaking Changes Analysis (ALWAYS RUNS)](#breaking-changes-analysis-always-runs)
  - [Database Safety Validation (CONDITIONAL)](#database-safety-validation-conditional)
  - [Component Impact Analysis (CONDITIONAL)](#component-impact-analysis-conditional)
  - [Dependency Graph Validation (CONDITIONAL)](#dependency-graph-validation-conditional)
  - [Test Coverage Validation (CONDITIONAL)](#test-coverage-validation-conditional)
  - [Infrastructure Safety (CONDITIONAL)](#infrastructure-safety-conditional)
  - [Traceability Validation (CONDITIONAL)](#traceability-validation-conditional)
- [Phase 4: Synthesize & Rewrite](#phase-4-synthesize-rewrite)
  - [Rewrite Format](#rewrite-format)
- [Phase 5: Conditional Plan Update](#phase-5-conditional-plan-update)
- [Report](#report)
  - [Report Format A: Issues Found](#report-format-a-issues-found)
  - [Report Format B: No Issues Found](#report-format-b-no-issues-found)
- [Error Handling](#error-handling)

Intelligently analyze an implementation plan in two passes: first determine whether the plan is fundamentally feasible for the current codebase, then run only the validations that actually apply. Detects missing prerequisites, nonexistent file references, architecture mismatches, breaking changes, database risks, component impacts, dependency issues, and more. If the plan is not feasible, stop validation and hand off to brainstorming/re-planning instead of trying to salvage the plan. When issues are found in otherwise feasible plans, automatically rewrite risky steps with safer alternatives while preserving the original as reference.
...
2. **Verify understanding** — use `ask the user` to confirm plan intent with 2-3 focused questions about goals, constraints, and expected outcome
3. **Feasibility preflight** — verify the plan is implementable in this repo before running validation workers
4. **Baseline provenance preflight** — validate `Validation Commands` file references exist in the current workspace
5. **Smart analysis** — analyze the plan to determine required validations
6. **Targeted validation** — run only relevant validations based on analysis
7. **Synthesize and rewrite** — collect validation results, assess risks, rewrite risky steps with safer alternatives
...
- Use subagents for selected validations only when the user explicitly asked for delegated or parallel validation and the current session supports it. Otherwise run the validations locally.
...
3. **Verify Understanding with User**
   Use `ask the user` with type: select to confirm you understand the plan's intent. Ask 2-3 vibe-coder-friendly questions focused on goals, not technical details:
...
- Before running any validation worker, perform a direct feasibility pass against the current codebase.
...
- **REQUIRED HANDOFF:** For `not-feasible` plans, invoke the brainstorming workflow for re-planning.
- Pass the structured feasibility result as the handoff context. At minimum include: `status`, `critical_blockers`, `missing_prerequisites`, `scope_concerns`, `repo_fit_summary`, and the original failed plan goal.
...
- Use `shell` to verify each referenced path exists in the current workspace.
...
4. **Determine Required Validations**
   ...
   Load `ValidationTypes.md` from the skill root for validation type definitions.

   Analyze the plan and determine which validations are needed:
...
Run only the required validations identified in Phase 2. If the user explicitly requested delegated or parallel validation and the current session supports it, these validations may run in subagents; otherwise run them locally.

### Breaking Changes Analysis (ALWAYS RUNS)
...
### Database Safety Validation (CONDITIONAL)
...
### Component Impact Analysis (CONDITIONAL)
...
### Dependency Graph Validation (CONDITIONAL)
...
### Test Coverage Validation (CONDITIONAL)
...
If any Validation Command references missing files, report whether this appears to be a missing-file issue.
Report: tests that will break, coverage gaps, and provenance findings.

### Infrastructure Safety (CONDITIONAL)
...
### Traceability Validation (CONDITIONAL)
...
12. **Collect Validation Results**
    - Gather results from local validation passes or any explicitly requested subagents
...
14. **Rewrite Risky Steps** (ONLY if issues found)
    ...
    - Write a new safer version below with explanation
...
- If feasibility preflight finds the plan is not implementable as written: stop worker fan-out, summarize the blockers in the structured feasibility result, and hand off to brainstorming for re-planning
- If validation analysis fails: report which analysis failed and continue with available results
