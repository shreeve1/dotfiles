...
### 0D. Focus Areas
ask_user: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. I'll generate visual mockups next, then review all 7 dimensions. Want me to focus on specific areas instead of all 7?"
...
After the board is serving, use ask_user to wait for the user. Include the
...
**After the user responds to ask_user:**
...
ask_user response instead of using the board. Use their text response
as the feedback.
...
Use ask_user to verify before proceeding.
...
**Do NOT use ask_user to ask which variant the user picked.** Read `feedback.json` — it already contains their preferred variant, ratings, comments, and overall feedback. Only use ask_user to confirm you understood the feedback correctly, never to re-ask what they chose.
...
* **NEVER use ask_user to ask which variant the user prefers.** Always create a comparison board first (`$D compare --serve`) and open it in the browser. The board has rating controls, comments, remix/regenerate buttons, and structured feedback output. Use ask_user ONLY to notify the user the board is open and wait for them to finish — not to present variants inline and ask "which do you prefer?" That is a degraded experience.
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
