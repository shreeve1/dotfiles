...
[ -z "$_AUTO" ] && _AUTO=$(~/.pi/agent/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
echo "AUTO_UPGRADE=$_AUTO"
```

**If `AUTO_UPGRADE=true` or `AUTO_UPGRADE=1`:** Skip ask_user. Log "Auto-upgrading gstack v{old} → v{new}..." and proceed directly to Step 2. If `./setup` fails during auto-upgrade, restore from backup (`.bak` directory) and warn the user: "Auto-upgrade failed — restored previous version. Run `/gstack-upgrade` manually to retry."

**Otherwise**, use ask_user:
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.pi/agent/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.pi/agent/skills/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.pi/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.pi/skills/gstack"
  ...
  INSTALL_DIR=".pi/skills/gstack"
elif [ -d "$HOME/.pi/skills/gstack" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.pi/skills/gstack" ]; then
...
_TEAM_MODE=$(~/.pi/agent/skills/gstack/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .pi/skills/gstack/ 2>/dev/null || true
if ! grep -qF '.pi/skills/gstack/' .gitignore 2>/dev/null; then
  echo '.pi/skills/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
