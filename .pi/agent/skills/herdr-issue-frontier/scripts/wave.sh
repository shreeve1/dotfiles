...
[[ "${HERDR_ENV:-}" == "1" ]] || { echo "FATAL: HERDR_ENV != 1 (run inside a herdr session)" >&2; exit 2; }
[[ -f "$v2" ]]                || { echo "FATAL: herdr-orchestration.sh not found: $v2 (set HERDR_ORCH_SCRIPT)" >&2; exit 2; }
