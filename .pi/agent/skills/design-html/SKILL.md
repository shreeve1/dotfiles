...
Then check for prior finalized.html. If `FINALIZED` was also found, use ask_user:
...
3. If in plan-driven or freeform mode (no approved PNG), design from context:
   ...
   - **Freeform:** use ask_user to gather what the user wants to build. Ask about:
...
If a framework is detected, use ask_user:
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.pi/skills/gstack/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/.pi/skills/gstack/design-html/vendor/pretext.js"
[ -z "$_PRETEXT_VENDOR" ] && [ -f ~/.pi/agent/skills/gstack/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=~/.pi/agent/skills/gstack/design-html/vendor/pretext.js
...
LOOP:
  ...
  3. ask_user (adjust wording based on mode):
...
Maximum 10 iterations. If the user hasn't said "done" after 10, use ask_user:
