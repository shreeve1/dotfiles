...
Then SHOW it to the user via ask_user:
...
Load the "## Pass 1" section from `~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`
...
3. **ask_user per friction point.** One question per friction point found.
...
7. ask_user if there's a genuine DX choice to resolve
...
~/.pi/agent/skills/gstack/bin/gstack-review-read 2>/dev/null | grep plan-devex-review || echo "NO_PRIOR_DX_REVIEWS"
```
...
Load reference: Read the "## Pass 1" section from `~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
**STOP.** ask_user once per issue. Recommend + WHY. Reference the persona.
...
Load reference: Read the "## Pass 2" section from `~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
**STOP.** ask_user once per issue. Recommend + WHY.
...
Load reference: Read the "## Pass 3" section from `~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 4" section from `~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 5" section from `~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 6" section from `~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 7" section from `~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 8" section from `~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
`~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.

Check each item. For any unchecked item, explain what's missing and suggest the fix.

**STOP.** ask_user for any item that requires a design decision.
...
Follow the ask_user format from the Preamble above. Additional rules for
DX reviews:

* **One issue = one ask_user call.** Never combine multiple issues.
...
If any ask_user goes unanswered, note here. Never silently default.
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-review-log '{"skill":"plan-devex-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW_CURRENT","tthw_target":"TTHW_TARGET","mode":"MODE","persona":"PERSONA","competitive_tier":"TIER","pass_scores":{"getting_started":N,"api_design":N,"errors":N,"docs":N,"upgrade":N,"dev_env":N,"community":N,"measurement":N},"unresolved":N,"commit":"COMMIT"}'
```
...
Use ask_user with applicable options:
