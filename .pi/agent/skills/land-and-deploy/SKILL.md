...
to guide deploy verification. If nothing is detected, ask the user via ask_user
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-review-read 2>/dev/null
```
...
Present the full dry-run results to the user via ask_user:
...
```bash
cat ~/.pi/agent/skills/gstack/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
eval $(~/.pi/agent/skills/gstack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
4. If no deploy workflows detected and no URL provided: use ask_user once:
...
If deploy fails (`conclusion` is `failure`): use ask_user:
...
If any fail: show the evidence (screenshot path, console errors, perf numbers). Use ask_user:
