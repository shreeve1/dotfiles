...
## Important: Use Bash with grep or rg for all code searches
...
**Code surface:** Use Bash with grep or rg to find endpoints, auth boundaries, external integrations, file upload paths, admin routes, webhook handlers, background jobs, and WebSocket channels. Scope file extensions to detected stacks from Phase 0. Count each category.
...
**Tier 2 — global skills (requires permission):** Before scanning globally installed skills or user settings, use ask_user:
...
For each OWASP category, perform targeted analysis. Use Bash with grep or rg for all searches — scope file extensions to detected stacks from Phase 0.
...
2. Use Bash with grep or rg to search for the same pattern across all relevant files
3. Report variants as separate findings linked to the original: "Variant of Finding #N"

**Parallel Finding Verification:**

For each candidate finding, launch an independent verification sub-task using the subagent tool. The verifier has fresh context and cannot see the initial scan's reasoning — only the finding itself and the FP filtering rules.
...
If the subagent tool is unavailable, self-verify by re-reading code with a skeptic's eye. Note: "Self-verified — independent sub-task unavailable."
...
**Remediation Roadmap:** For the top 5 findings, present via ask_user:
