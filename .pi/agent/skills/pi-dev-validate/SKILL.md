...
Intelligently analyze an implementation plan by running only the validations that actually apply. Detects breaking changes, database risks, component impacts, dependency issues, and more. When issues are found, automatically rewrite risky steps with safer alternatives while preserving the original as reference. Feasibility is handled upstream by `pi-dev-plan`'s Phase 3.5 preflight; this skill assumes plans reaching it are feasible and focuses on risk analysis and rewriting.
...
- **Feasibility is already covered**: `pi-dev-plan` performs a feasibility preflight before producing the plan. This skill focuses on risk analysis and rewriting.
...
13. **Risk Assessment**
    ...
    - Critical: Will definitely break existing functionality, or reveals a feasibility issue `pi-dev-plan` missed
...
- If validation surfaces critical feasibility issues `pi-dev-plan` missed (nonexistent edit targets, missing foundations, impossible sequencing): report them as critical findings and recommend the user return to `pi-dev-plan` or `pi-brainstorm` rather than proceeding to build. If the blocker is multi-epic scope crammed into one plan and a parent PRD exists, recommend `pi-dev-epic` for decomposition instead.
