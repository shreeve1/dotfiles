...
**ask_user with pre-filled context:** Pre-fill what you inferred from the codebase,
...
Use ask_user to confirm before spending API credits:
...
After receiving feedback (via HTTP POST or ask_user fallback), output a clear
...
Use ask_user to confirm before saving.
...
If standalone, offer next steps via ask_user:
