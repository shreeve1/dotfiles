...
const RESERVED_TEMPLATE_NAMES = new Set([
	"task",
	"previous",
	"chain_dir",
	"outputs",
...
const DYNAMIC_EXPAND_KEYS = new Set([
	"from",
	"item",
	"key",
	"maxItems",
...
export function assertJsonPointer(pointer: string, label: string): void {
	...
	if (!pointer.startsWith("/")) {
		throw new DynamicFanoutError(
			`${label} must be a JSON Pointer starting with '/'.`,
		);
	}
	for (const segment of pointer.slice(1).split("/")) {
		if (/~(?![01])/.test(segment)) {
			throw new DynamicFanoutError(
				`${label} contains invalid JSON Pointer escape.`,
			);
		}
	}
}
...
export function resolveItemTemplate(
	...
	return template.replace(
		ITEM_REF_PATTERN,
		(raw, name: string, pathText: string | undefined) => {
			if (name !== itemName) return raw;
			if (
				pathText !== undefined &&
				(!pathText.trim() || pathText.includes(".."))
			) {
...
export function assertNoUnresolvedItemReferences(
	...
	for (const match of template.matchAll(/\{([^{}]*)\}/g)) {
		...
		if (reference === itemName || reference.startsWith(`${itemName}.`)) {
			if (
				!ITEM_REF_PATTERN.test(raw) ||
				reference === `${itemName}.` ||
				reference.includes("..")
			) {
				throw new DynamicFanoutError(
					`Invalid item reference '${raw}' in ${label}.`,
				);
			}
...
export function validateDynamicStepShape(
	...
	if (!ITEM_NAME_PATTERN.test(itemName))
		throw new DynamicFanoutError(
			`${prefix} has invalid expand.item '${itemName}'.`,
		);
	...
	if (
		...
		throw new DynamicFanoutError(
			`${prefix} expand.maxItems must be an integer >= 0.`,
		);
	}
	if (
		...
		throw new DynamicFanoutError(
			"config.chain.dynamicFanout.maxItems must be an integer >= 0.",
		);
	}
	...
	assertOnlyKeys(
		step.parallel,
		config.allowRunnerFields
			? RUNNER_DYNAMIC_PARALLEL_KEYS
			: DYNAMIC_PARALLEL_KEYS,
		`${prefix} parallel`,
	);
	if ("expand" in (step.parallel as object))
		throw new DynamicFanoutError(
			`${prefix} does not support nested dynamic fanout.`,
		);
	...
	for (const [label, template] of [
		...
		if (template)
			assertNoUnresolvedItemReferences(
				template,
				itemName,
				`${prefix} ${label}`,
			);
...
export function materializeDynamicParallelStep(
	...
	if (items.length === 0) {
		if ((step.expand.onEmpty ?? "skip") === "fail") {
			throw new DynamicFanoutError(
				`Dynamic chain step ${stepIndex + 1} source array is empty.`,
			);
		}
...
export function collectDynamicResults(
	...
	results: Array<
		Pick<
			...
			| "artifactPaths"
			| "savedOutputPath"
		> & { output?: string; finalOutput?: string }
	>,
...
export function validateDynamicCollection(
	...
	if (validation.status === "invalid") {
		throw new DynamicFanoutError(
			`Collected output validation failed: ${validation.message}`,
		);
	}
}
