...
3. Every Harness Profile entry row for languages, tools, checks, formatters, lint, guidance, prompt advisories, git reminders, or smoke tests must contain one of `selected`, `selected_if_json`, `skipped`, or `not_detected`. Do not apply this rule to headings or explanatory prose.
...
12. Smoke commands must be explicit in profile. Record load smoke, isolated load smoke, syntax dry checks, and guidance dry selections in `smokeTests`.
...
5. Syntax dry checks when `PROFILE.smokeTests.syntaxDryChecks` is non-empty, using temporary files generated from the matching `PROFILE.syntaxChecks` entries:
   ...
   # Current dotfiles profile example:
   printf '{"ok":true}\n' > "$tmpdir/harness.json"
...
6. Guidance dry check when `PROFILE.smokeTests.guidanceDry` is true:
