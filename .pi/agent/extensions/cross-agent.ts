...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    const guidance = loadClaudeGuidance(home, ctx.cwd);
    if (groups.length === 0 && guidance.length === 0) return;
    ...
    setTimeout(() => {
      ...
      const pushCard = (title: string, counts: string[], body: string) => {
        if (cardCount > 0) lines.push("");
        cardCount++;
        const countStr = counts.length ? "  " + counts.join(" ") : "";
        lines.push(pink(bold(`  ${title}`)) + countStr);
        lines.push(pad);
        ...
        if (wrapped.length > maxRows) {
          const overflow = dim(`  ... ${wrapped.length - maxRows} more`);
      ...
      if (guidance.length) {
        const importCount = guidance.reduce(
          (sum, file) => sum + file.imports.filter((i) => i.status === "loaded").length,
          0,
        );
        const counts = [
          yellow("(") +
            green(`${guidance.length}`) +
            dim(` guidance file${guidance.length > 1 ? "s" : ""}`) +
            yellow(")"),
        ];
        if (importCount) {
          counts.push(
        ...
        pushCard(
          "Claude guidance",
          counts,
          guidance.map((g) => cyan(g.label)).join(yellow(", ")),
        );
      }
      ...
      for (const g of groups) {
        ...
        if (g.skills.length)
          counts.push(
            yellow("(") +
              green(`${g.skills.length}`) +
              dim(` skill${g.skills.length > 1 ? "s" : ""}`) +
              yellow(")"),
          );
        ...
        if (g.agents.length)
          counts.push(
            yellow("(") +
              green(`${g.agents.length}`) +
              dim(` agent${g.agents.length > 1 ? "s" : ""}`) +
              yellow(")"),
          );
        ...
        if (g.agents.length) {
          items.push(
            yellow("/agent:") +
              g.agents.map((a) => green(a.name)).join(yellow(", /agent:")),
          );
        }

        pushCard(g.source, counts, items.join("\n"));
      }
