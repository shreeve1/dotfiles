/**
 * Cross-Agent — Load commands and skills from Claude Code setup
 *
		...
		.map((line) => {
			...
			if (wholeLine) {
				...
				if (
					!wholeLine[2] &&
					!wholeLine[3] &&
					!shouldTreatAsInlineImport(spec)
				) {
					return line;
				}
...
function loadClaudeGuidance(home: string): GuidanceFile[] {
	const seen = new Set<string>();
...
function formatSourceReport(
	...
	if (groups.length === 0) {
		...
		for (const g of groups) {
			lines.push(
				`- ${g.source}: ${g.commands.length} commands, ${g.skills.length} skills`,
			);
		}
	}
...
export default function (pi: ExtensionAPI) {
	...
	for (const source of collectClaudeDirs(home, cwd)) {
		...
		if (commands.length || skills.length) {
			groups.push({ source: source.label, commands, skills });
		}
	}
	...
	// Register Claude commands once — never re-registered on /new.
	// Agent handoff belongs to pi-subagents' Agent tool, not raw /agent:name bridges.
