...
// Effort levels
const BASE_EFFORT_LEVELS: ThinkingLevel[] = [
	"minimal",
	"low",
	"medium",
	"high",
...
const EFFORT_ORDINAL: readonly ThinkingLevel[] = [
	"minimal",
	"low",
	"medium",
	"high",
...
const MSG_PERSIST_FAILED =
	"Failed to save advisor selection — selection not persisted";
...
interface AdvisorConfig {
	...
	disabledForModels?: Array<
		string | { model: string; minEffort?: ThinkingLevel }
	>;
}
...
function validateDisabledForModels(
	value: unknown,
): Array<string | { model: string; minEffort?: ThinkingLevel }> {
	if (!Array.isArray(value)) return [];
	return value.filter(
		(entry): entry is string | { model: string; minEffort?: ThinkingLevel } => {
			if (typeof entry === "string") return entry.length > 0;
			if (typeof entry !== "object" || entry === null) return false;
			const obj = entry as Record<string, unknown>;
			if (typeof obj.model !== "string" || obj.model.length === 0) return false;
			if (
				obj.minEffort !== undefined &&
				!EFFORT_ORDINAL.includes(obj.minEffort as ThinkingLevel)
			)
				return false;
			return true;
		},
...
let disabledForModelsCache: Array<
	string | { model: string; minEffort?: ThinkingLevel }
> = [];

export function setDisabledForModels(
	models: Array<string | { model: string; minEffort?: ThinkingLevel }>,
): void {
...
export function restoreAdvisorState(
	...
	if (isExecutorBlocked(ctx, pi.getThinkingLevel())) {
		if (ctx.hasUI) {
			...
			ctx.ui.notify(
				msgAdvisorRestoredInactive(advisorLabel, config.effort),
				"info",
			);
		}
		return;
	...
	if (ctx.hasUI) {
		ctx.ui.notify(
			msgAdvisorRestored(`${model.provider}:${model.id}`, config.effort),
			"info",
		);
	}
}
...
async function executeAdvisor(
	...
	const messages: Message[] = inventoryMessage
...
const ADVISOR_DESCRIPTION =
	"Escalate to a stronger reviewer model for guidance — use EXCEPTIONALLY when " +
	"stuck, when evidence conflicts, or when changing approach (not by default). " +
	"Call when you need stronger judgment on a complex decision, an ambiguous " +
	"failure, a problem you're circling without progress, or a risky shift in " +
	"direction; then resume. Takes NO parameters — when you call advisor(), your " +
	"entire conversation history is automatically forwarded. The advisor sees the " +
	"task, every tool call you've made, every result you've seen.";

export const DEFAULT_PROMPT_SNIPPET =
	"Escalate to a stronger reviewer model for guidance when stuck, on conflicting evidence, or when changing approach";

export const DEFAULT_PROMPT_GUIDELINES: string[] = [
	"Call `advisor` EXCEPTIONALLY, not by default. Don't pre-call it before substantive work; don't pre-call it before declaring done.",
	"Do call `advisor` when STUCK — errors keep recurring, approach isn't converging, results don't fit.",
	"Do call `advisor` when EVIDENCE CONFLICTS — you've found primary-source data pointing one way and another source (or your own reasoning) points another. Surface the conflict: 'I found X, you suggest Y, which constraint breaks the tie?' A reconcile call is cheaper than committing to the wrong branch.",
	"Do call `advisor` when about to CHANGE APPROACH or take an UNUSUALLY RISKY decision.",
	"BEFORE any `advisor` call, make your work durable: write the file, save the result, commit the change. The advisor call takes time; if the session ends during it, a durable result persists and an unwritten one doesn't.",
...
export function registerModelSelectHandler(pi: ExtensionAPI): void {
	pi.on("model_select", async (event, ctx) => {
		...
		if (blocked && hasTool) {
			...
			if (ctx.hasUI) {
				ctx.ui.notify(
					msgAdvisorRestored(modelKey(advisor), getAdvisorEffort()),
					"info",
				);
			}
		}
...
function isModelBlocked(
	...
	for (const entry of disabledForModelsCache) {
		if (typeof entry === "string") {
			...
			const executorOrdinal = EFFORT_ORDINAL.indexOf(
	...
	pi.registerCommand("advisor", {
		...
		handler: async (_args, ctx) => {
			...
			if (picked.reasoning) {
				...
				effortChoice =
					effortResult === OFF_VALUE
						? undefined
						: (effortResult as ThinkingLevel);
			}
			...
			if (blocked) {
				ctx.ui.notify(
					msgAdvisorEnabledInactive(modelKey(picked), effortChoice),
					"info",
				);
			} else {
				ctx.ui.notify(
					msgAdvisorEnabled(modelKey(picked), effortChoice),
					"info",
				);
			}
		},
