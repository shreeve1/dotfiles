...
export const WORKFLOW_TOOL_DESCRIPTION = [
  ...
  "• await agent(prompt, { label?, phase?, schema?, model?, provider?, effort?, writable? }) — run ONE subagent in an isolated context and wait for it. Always resolves to { ok, output, structured?, error? }. Check `ok` before using the result. When you pass a JSON `schema`, `structured` holds the validated object on success. `model`/`provider` override the session model; `effort` sets the thinking level (off|minimal|low|medium|high|xhigh|max); `writable: true` opts the child back into `write`/`edit` (children are read-only by default). Children receive normal built-ins and trust-appropriate extensions, settings, skills, and AGENTS.md context, but cannot recursively orchestrate or ask the user.",
