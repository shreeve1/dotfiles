...
export function collectInstallDiagnostics() {
    const notes = [];
    ...
    const astGrepCli = safe(() => {
        ...
        return ["ast-grep", "ast-grep.exe", "sg", "sg.exe"].some((b) => fs.existsSync(path.join(dir, b)));
    }, false);
    const grammars = safe(() => {
...
export function formatInstallDiagnostics(diag = collectInstallDiagnostics(), cause) {
    const lines = [];
