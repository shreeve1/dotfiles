 ...
 */
export function parseUntrackedIgnoredOutput(stdout, cwd) {
    const ids = new Set();
...
async function fetchUntrackedIgnoredIds(cwd) {
    try {
        const result = await safeSpawnAsync("git", ["ls-files", "--others", "--ignored", "--exclude-standard"], { cwd, timeout: 10_000, resourceLabel: "git-tracked-ignore" });
 ...
 * contract.
 */
export function collectUntrackedIgnoredIds(cwd) {
...
export function _resetUntrackedIgnoredCacheForTests() {
    _cache.clear();
}
/**
 ...
 */
export function parseTrackedFilesOutput(stdout, cwd) {
    const ids = new Set();
...
const _trackedSnapshot = new Map();
/**
 ...
 */
export function collectTrackedFiles(cwd) {
