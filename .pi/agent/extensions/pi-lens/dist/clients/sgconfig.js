 ...
 */
export function getAstGrepRuleSources(projectRoot = process.cwd()) {
...
function parseRuleId(raw) {
    ...
    const unquoted = (withoutComment.startsWith('"') && withoutComment.endsWith('"')) ||
...
const bundledSnapshots = new Map();
...
function materializeMergedRuleDir(sources, mergedDir) {
    ...
    const claimedByEarlierSource = new Set();
    for (const [sourceIndex, source] of sources.entries()) {
        const claimedThisSource = new Set();
        for (const [fileIndex, file] of source.files.entries()) {
            ...
            if (survivingDocuments.length === file.documents.length) {
                ...
                fs.writeFileSync(destination, `${survivingDocuments
 ...
 */
export function resolveBaselineSgconfig(projectRoot = process.cwd()) {
    ...
    const protectedArtifacts = new Set([file, mergedDir]);
...
export function _resetBaselineSgconfigForTests() {
