    ...
    for (const dir of walkUpDirs(cwd)) {
        ...
        if (stat.isFile()) {
            ...
            try {
                ...
                if (!fs.statSync(linkedGitDir).isDirectory())
                    return null;
                return {
                    gitDir: linkedGitDir,
...
function getResolvedGitDir(cwd) {
    ...
    if (_gitDirCache.has(key))
        return _gitDirCache.get(key) ?? null;
    let resolved;
    try {
