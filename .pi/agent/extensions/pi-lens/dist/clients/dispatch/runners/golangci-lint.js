...
function parseGolangciJson(raw, filePath) {
    try {
        ...
        return output.Issues.filter((issue) => path.resolve(issue.Pos.Filename) === absFile).map((issue) => {
...
const golangciRunner = {
    ...
    async run(ctx) {
        ...
        const result = await safeSpawnAsync(cmd, ["run", "--out-format=json", ctx.filePath], { timeout: 60000, cwd });
        if (result.status === 0) {
