...
function buildForwardRegex(token, sep) {
...
export function analyzeMiddleMan(classEntry, lines, languageId) {
    ...
    const candidates = classEntry.members.filter((m) => {
        ...
        if (m.name === className || m.name === `~${className}`)
            return false; // C++/C# ctor/dtor-by-name
    ...
    for (const member of candidates) {
        ...
        if (paramNames === undefined)
            continue; // destructured params — ineligible
 ...
 * top-level ones.
 */
export function annotateMiddleMan(entries, content, languageId) {
