...
export function createLspDiagnosticsTool() {
    return {
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            if (Array.isArray(typedParams.filePaths) &&
...
async function collectDiagnosticsForFile(absPath, lspService, waitMs) {
    try {
    ...
    return lspService.getDiagnostics(absPath, waitMs !== undefined ? "document" : "full");
}
...
async function collectFileDiagnosticResult(file, severity, lspService, waitMs) {
    try {
    ...
    return {
        file,
        diagnostics: diagnosticsToFileDiags(file, applySeverityFilter(rawDiags, severity)),
        unavailable: lspUnavailableMessage(file, health),
    };
}
async function runFileDiagnostics(absPath, severity, lspService, waitMs) {
