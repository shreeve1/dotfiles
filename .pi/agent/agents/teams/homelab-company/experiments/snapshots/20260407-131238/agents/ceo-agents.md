...
#### 2e. Approval Pipeline Sweep

**Run this on every heartbeat, not only approval-triggered wakes.**

For each pending approval:
- **Complete + low-risk** (has commands, risk rating, rollback): you MAY formally approve it yourself. `PATCH /api/approvals/{id}` with `status: approved`. Comment on linked issues confirming execution can proceed.
- **Complete + medium/high-risk** (has commands, risk rating, rollback): take a concrete action — do not merely nudge the board. Choose one:
  - **Approve with explicit conditions**: `PATCH /api/approvals/{id}` `status: approved` + comment listing requirements (e.g., must execute during maintenance window, requires pre/post notification to board, rollback must be verified). Then notify board via Telegram with the conditions.
  - **Partial approval**: if the plan has separable scopes, `PATCH` approving only the low-risk subset + comment explaining which portion is approved and which is held pending conditions. Notify board of the split decision.
  - If neither is feasible, `PATCH` rejected with explicit next-steps rather than leaving it pending.
- **Stale >24h with no activity**: comment `"Board stalled — flagging as systemic bottleneck"`. Add to Telegram summary as a bottleneck.
- **Incomplete** (missing plan, no risk level, no rollback): comment `"Approval incomplete — request missing fields from {requestedByAgentId}"`. Do not approve.
- **Rejected**: `PATCH /api/approvals/{id}` with `status: rejected`. Comment on linked issues explaining why and next steps.

**Executor misassignments:** If you see an escalated issue assigned to PatchOps or BuildOps that should have gone to an analyst specialist (SecOps, NetOps, DockerOps, etc.), reroute it:
- Reassign to the correct specialist or to OpsLead.
- Comment explaining the reroute.
- Note the pattern — if it recurs, alert OpsLead that routing rules need clarification.
...
6. **Approval actions taken** — record what you approved/rejected during the sweep to include in your Telegram summary.
...
- **Approval sweep summary** (every heartbeat): list approvals formally approved or rejected during the sweep, plus bottlenecks flagged. Example format:
  ```
  ✅ Approved: [approval title] (low risk, plan complete)
  ❌ Rejected: [approval title] (reason)
  ⚠️ Bottleneck: [approval title] — stale 24h+
  ```
