...
Categorize each pending approval:
- **Complete + low-risk** (has commands, risk, rollback): `PATCH /api/approvals/{id}` with `status: approved`. Comment on linked issues.
- **Complete + medium/high-risk** (has commands, risk, rollback): take a concrete action — do not merely nudge the board. Choose one:
  - **Approve with explicit conditions**: `PATCH /api/approvals/{id}` `status: approved` + comment listing requirements (e.g., must execute during maintenance window, requires pre/post notification to board, must have rollback verified). Then notify board via Telegram with the conditions.
...
- **Stale >24h with no activity**: comment `"Board stalled — flagging as systemic bottleneck"`.
- **Incomplete**: comment requesting missing fields. Do not approve.
- **Rejected**: `PATCH /api/approvals/{id}` with `status: rejected`. Comment explaining why and next steps.

Also check for executor-misassigned escalated issues — reroute to OpsLead or the correct analyst specialist with a comment.
