...
#### Review stale todo issues — detect bounce patterns and stale work:
1. **Get all `todo` issues** assigned to any specialist:
   ```
   GET /api/companies/{companyId}/issues?status=todo
   ```
2. **Executor bounce pattern (immediate, no age threshold):** If an issue has 2+ comments from an executor (PatchOps/BuildOps) saying "no approved plan" or "returning to idle" — reassign to the original investigating specialist (SecOps, NetOps, StorageOps, DockerOps, MediaOps) with an explicit ask to investigate and produce a plan. Do not leave it with the executor.
3. **Stale medium/high priority or investigation-start:** Any `todo` issue with priority `medium` or `high`, or with a comment showing investigation has started, that has no substantive update for >12h — ping the assignee or reassign.
4. **Approval-ready issues:** If a specialist has posted a plan with commands/risk/rollback but no approval exists, create the formal approval yourself (see below). Flag high-risk plans for CEO/board instead.
5. **Distinguish missing investigation vs missing approval:**
   - **Missing investigation:** No plan, no findings, no analysis yet. → Reassign to the appropriate specialist with an explicit ask.
   - **Missing approval:** Plan with commands/steps, risk level, and rollback documented. → **Create a formal approval yourself** (see below).
6. **Reassign executor-bounced issues** back to the right investigator — if the issue was returned from PatchOps/BuildOps as not their responsibility or unexecutable, route it back to the originating specialist (SecOps, NetOps, etc.) with a note explaining why.

#### Create formal approvals for low-risk complete plans:
If a specialist has provided a plan that meets ALL of the following, create the approval yourself:
- ✅ Concrete commands or steps
- ✅ Risk level stated (e.g., "low", "medium")
...
**Escalate to CEO/board if the plan is incomplete or high-risk:**
If the specialist plan is missing commands, has no risk assessment, or is rated high/medium risk:
- Comment on the issue: `"Plan incomplete or high-risk — escalating to board."`
- **Do not create the approval yourself.** Instead, create a todo issue assigned to the CEO flagging the issue and requesting board review.
- Send Telegram summarizing the high-risk or incomplete plan.
