...
Hosts: pve1-4, running LXC/VMs. Skip TrueNAS and Wazuh (own runbooks). Apply Severity Thresholds.
...
### Check 2: qBittorrent (Docker in CT 108 on pve1, port 8090)
...
Process check fallback: `pct exec 108 -- docker ps --format '{{.Names}}' | grep -q qbittorrent`

If not active → **high** issue.

### Check 3: Jellyseerr (CT 107 on pve2 — use pct exec, not direct SSH)
...
### Check 6: Pi-hole FTL Errors (ignore transient `Database is busy`)
...
### Check 8: UDM Pro System Errors (known: `inadyn` Cloudflare errors — only flag if new)
...
**Project:** Infrastructure (from Runbook Routing table)

Map monitor type to the corresponding runbook's assignee from the Runbook Routing table:
- Host/Proxmox → Infrastructure (OpsLead) | Docker container → Docker (DockerOps) | Network/DNS/VPN → Network (NetOps) | Storage/backup → Storage (StorageOps) | Media stack → Media (MediaOps) | Security → Security (SecOps)
- Patching needed → PatchOps (`54135f3c-9778-4209-9498-7e2c50424acf`)

Steps: Parse webhook JSON → SSH to affected host to investigate → create issue (severity: `down=high`, `degraded=medium`) with findings + original payload → assign to correct specialist.
