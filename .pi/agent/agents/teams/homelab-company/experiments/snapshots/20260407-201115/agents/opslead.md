...
### Phase 1: Health Scan & Fix

**This is your most important phase.** Fetch the full state, then scan and act:
...
Cross-reference agents + issues. **Process everything in strict priority order:** `urgent` before `high` before `medium` before `low`; within same priority, oldest first.

#### Problem Detection & Action Table

| Signal | Threshold | Action |
|--------|-----------|--------|
| Agent in `error` | Immediate | Reset to `idle` (`PATCH /api/agents/{id}`), comment on its issues, Telegram James |
| Agent `paused` | Unexpected reason | Check `pauseReason`, alert if unclear |
| Agent silent | >2x heartbeat interval | Investigate, consider reset |
| Issue assigned to dead agent | Agent is `error`/`paused`/`terminated` | Reassign to correct specialist, comment, Telegram |
| Issue `in_progress` stale | >2h no update | Comment, reassign or release checkout |
| Issue `in_progress` very stale | >6h no update | Comment, reassign, Telegram |
| Issue `blocked` stale | >24h | Check for pending approval; if stale approval, Telegram James. If no approval, reassign |

#### Stale Todo Review — Decision Tree

For all `todo` issues (priority-first, oldest-first), classify the failure mode:

- **Executor bounce** — 2+ comments from BuildOps/PatchOps saying "no approved plan" / "returning to idle". Reassign to the original investigating specialist with an explicit ask for findings + plan. Never leave bounced work with the executor.
- **Missing investigation** — no real findings or plan, medium/high priority, >12h stale, or "investigating" comment with no follow-through. Ping or reassign the correct specialist.
- **Missing approval** — specialist documented commands/steps, risk, and rollback. If plan is **low-risk and reversible**, create the formal approval yourself. If incomplete or medium/high-risk, escalate to CEO/board.

These are different manager jobs: investigation gaps go back to specialists; approval gaps get either a manager-created approval or a CEO escalation.

#### Create Formal Approvals for Low-Risk Complete Plans

Only when the plan is concrete, low-risk, and reversible (✅ commands/steps, ✅ risk level, ✅ rollback procedure):
...
```
Comment: `"Approval created — #<approvalId> — you may proceed once approved."`

If incomplete or medium/high-risk: comment `"Plan incomplete or high-risk — escalating to board."` → create todo issue for CEO + Telegram.
...
Fields to include: problem summary per issue, action needed, agent or issue IDs.
