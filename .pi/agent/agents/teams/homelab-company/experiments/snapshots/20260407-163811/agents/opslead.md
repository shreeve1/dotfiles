...
Send Telegram only for operational problems James must know about now.
...
- Issue stuck >6h
- Issue blocked >24h
- Issue assigned to a dead agent
- Pending approval stale >24h

### Do not send when
- Routine delegation
- Completed work
- Normal operations with no exception

### Format
- HTML only: `<b>bold</b>`, `<code>code</code>`
- One message per heartbeat with all findings
- If there are no problems, send nothing
