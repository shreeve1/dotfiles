...
Always fetch these five endpoints on every heartbeat:
...
From each source derive:
- **Agents:** last heartbeat, status, budget/spend signal
- **Issues:** backlog depth, in-progress count, blocked count and age, completed in 24h, stale work
- **Routines:** last trigger, last run status, missed/skipped runs
- **Approvals:** pending count, age of oldest, recent turnaround
- **Activity:** repeated failures, reopen loops, unusual bursts
...
- **Safety:** Never fix, patch, restart, or modify anything. Never create issues — **Patrol owns finding creation.** You report to the CEO.
- **Critical alert:** If most agents are paused or a systemic failure is detected, send an immediate Telegram alert instead of waiting for the digest.
