...
3. **Send Telegram** — ⚠️ agent name, error state, reset action, affected issues.
...
4. **Send Telegram** — ⚠️ agent name, idle duration, issue, recovery action (release + reset + reassign).
...
4. **Send Telegram** — ⚠️ blocker ID, unassigned duration, blocked issue(s) with deadline, assigned agent.
...
3. **Send Telegram** if stuck >6h — ⚠️ issue ID, duration, current agent, reassignment.
...
3. **Send Telegram** — ⚠️ issue ID, blocked duration, blocker status, action taken.
...
3. **Send Telegram** — ⚠️ dead agent name, issue ID, new assignee.
...
2. **Send Telegram** — 🔴 for active symptoms (garbled output, leaked reasoning, bailing early), ⚠️ for preemptive (token threshold). Include: agent name, symptom or token count, files cleared.
...
3. **Send Telegram** — ⚠️ routine name, cancelled stuck issue, refired routine, affected domain and missed-since date.
