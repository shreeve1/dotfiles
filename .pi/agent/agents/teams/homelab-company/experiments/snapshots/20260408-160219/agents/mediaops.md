...
- **One service investigation per run.**
...
4. **Check `para-memory-files` for prior incidents on the same host, service, or symptoms.** If a match is found:
   ...
   - If the prior fix applies, execute autonomously (restart/cache clear) or create an approval immediately referencing the precedent
   - If the prior fix does NOT apply (different root cause), explain why before proceeding
5. Investigate the affected service — apply cascading-failure tracing if errors form a chain
...
Key services: Jellyfin (10.20.20.20:8096), Sonarr (10.20.20.23:8989), Radarr (10.20.20.24:7878), Prowlarr (10.20.20.26:9696), qBittorrent (10.20.20.25:8090), Jellyseerr (10.20.20.27:5055), FlareSolverr (10.20.20.22)
...
- Prior media stack incidents and their resolution (service, host, fix applied, approval ID)
