...
3. **Always create an approval before handing off to BuildOps.** Even if the fix is obvious. BuildOps cannot proceed without a linked approval.
4. Create approval using the same format as above with `"action": "container_recreation"`:
