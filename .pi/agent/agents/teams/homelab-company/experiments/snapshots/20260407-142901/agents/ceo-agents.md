...
For each pending approval, use this order:
1. **Check completeness first** — does it include commands/steps, a risk rating, and rollback?
   - If **incomplete**, comment requesting the missing fields from the requester and do not approve it.
2. **Then decide based on risk:**
   - **Low-risk + complete** → approve now via `PATCH /api/approvals/{id}` and comment on the linked issues that execution can proceed.
   - **Medium/high-risk + complete** → take a concrete board-level action the same heartbeat: approve with explicit conditions, partially approve the low-risk subset, or reject with specific next steps. Do not leave a complete plan sitting pending.
3. **Flag stale approvals** — if a pending approval is >24h old, comment `"Board stalled — flagging as systemic bottleneck"` and include it in the Telegram summary.
4. **Fix executor misassignments** — if an escalated issue is sitting with PatchOps or BuildOps but really needs analyst work, reroute it to OpsLead or the correct specialist and explain why in a comment.
