...
3. `GET /api/companies/{companyId}/issues/{taskId}` — checkout your assigned routine execution issue
4. **Read the issue title to determine which runbook to execute.** The title tells you which domain to check:
...
6. For each finding: check for duplicates before creating (see Rules)
7. Comment summary on routine issue, mark done, and exit

## Rules

- Read-only. Never modify anything on any host.
- Timeout all SSH at 10s: `ssh -o ConnectTimeout=10 -o BatchMode=yes`
...
**Project:** Security Operations (`8b7bdd7e-b862-4d88-adff-dbf8c029121c`) | **Goal:** Minimize Vulnerability Exposure (`4a5d67fc-1a29-431b-a520-f76892591b6e`) | **Assign to:** SecOps (`5229c112-eeba-40d8-b31f-4f00b5bcafab`)
...
**Project:** Infrastructure (`9a5bf8fd-4052-47e5-9f56-f46049b83f43`) | **Goal:** Maximize Service Uptime (`64345812-e6c8-49fb-9610-ad4ab69d76c8`) | **Assign to:** OpsLead (`7c040a50-5b26-4849-83f7-a110f07f6059`)
...
**Project:** Media Stack (`639d4fc5-a207-4fef-ba08-a9f146c0466d`) | **Goal:** Keep Media Stack Running (`228a8052-a7ac-4292-b419-8344fe641ebb`) | **Assign to:** MediaOps (`3f8b6a93-8d1d-42df-8a5b-729baf283a6b`)
...
**Project:** Network & DNS (`bb5173f0-079a-4e92-8067-b699f9bb2e4a`) | **Goal:** Network Reliability (`3948ccf9-c627-4b73-b86a-11dc69554d45`) | **Assign to:** NetOps (`dc9d6a93-ba6a-416e-8a48-10dfe4912909`)
...
**Project:** Storage & Backups (`c2d7a01f-5f42-489a-9359-93a5542757fa`) | **Goal:** Ensure Backup Integrity (`a0fc1de6-71ae-4fff-817c-82c57ae26c9d`) | **Assign to:** StorageOps (`5ff815f6-0ef3-4d1e-b2e7-c78594f271a1`)
...
**Project:** Infrastructure (`9a5bf8fd-4052-47e5-9f56-f46049b83f43`) | **Goal:** Docker Image Currency (`af35bf68-d26a-4620-a8ae-2d82dfb1cb15`) | **Assign to:** DockerOps (`04e0b743-a8b5-4ebe-9011-8fae774b6dce`)
...
**Assign to:** Determine based on monitor type:
...
curl -sS -X POST "$PAPERCLIP_API_URL/api/companies/$PAPERCLIP_COMPANY_ID/issues" \
  ...
  -d '{
    "projectId": "<from the runbook section above>",
    "goalId": "<from the runbook section above>",
    ...
    "assigneeAgentId": "<from the runbook section above>",
    "status": "todo",
