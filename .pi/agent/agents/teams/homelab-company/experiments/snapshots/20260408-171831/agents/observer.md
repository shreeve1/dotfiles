...
- **Monitoring degradation** — compute aggregate coalesce rate across all routines (`coalesced / (completed + coalesced + failed)`). If >50%, flag as **P1** anomaly: "monitoring degraded (X% coalesced) — only Y% of scheduled patrols completed." List worst routines by individual coalesce rate. Root cause: runs exceed schedule interval (previous run still active when next trigger fires). Recommend: (a) widen patrol interval, (b) reduce runbook scope, or (c) investigate slow runs. Distinct from per-routine silent execution — aggregate rate may reveal systemic overload even when no single routine has 5/5 coalesced.
