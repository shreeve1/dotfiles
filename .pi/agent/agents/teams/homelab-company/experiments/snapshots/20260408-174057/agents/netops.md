...
When an investigation surfaces multiple recommendations, classify each and create separate issues:

| Classification | Examples | Route to |
|----------------|----------|----------|
| **Low-risk** (diagnostics/docs only) | Pi-hole gravity refresh, cache tuning, DNS bypass validation, monitoring thresholds, documentation | NetOps (`$PAPERCLIP_AGENT_ID`) |
| **Approval-required** (live network changes) | UDM DNS, firewall/port forwarding, proxy rules, VPN tunnels, external routing | BuildOps after board approval |

1. Create each follow-up via `POST /api/companies/$PAPERCLIP_COMPANY_ID/issues` with `parentId: "$PAPERCLIP_TASK_ID"`, your project/goal IDs, and assignee per table above
2. For approval-required items: request approval on the follow-up issue, then assign to BuildOps (`55a1abf0-91fe-4d60-942b-da45390c0bc5`)
3. Close the parent issue with investigation summary + list of child issue IDs
...
3. **Create a new approval** with the correct fix — include `"supersedes_approval": "<old-id>"` in the payload and explain why the old plan was wrong
...
curl -sS -X POST "$PAPERCLIP_API_URL/api/companies/$PAPERCLIP_COMPANY_ID/approvals" \
  ...
  -d '{
    ...
    "payload": {
      ...
      "supersedes_approval": "<old-approval-id-if-replacing-wrong-plan>"
    },
    "issueIds": ["<issue-id>"]
  }'
```
