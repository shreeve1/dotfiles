...
1. **Get all `todo` issues** assigned to specialists:
   ...
   ```
2. For each todo issue, classify the failure mode before acting:
...
3. Treat these as different manager jobs: investigation gaps go back to specialists; approval gaps get either a manager-created approval or a CEO escalation.
