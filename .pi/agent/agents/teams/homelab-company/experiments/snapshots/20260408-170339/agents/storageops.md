...
When a pool is degraded (drive faulted, removed, or offline), your investigation must cover ALL of the following before creating the approval:

1. **Explain the redundancy tolerance.** State the pool topology (raidz1/raidz2/raidz3/mirror/stripe) and how many additional drive failures the pool can survive. For example: "pool1 is raidz2 (6 drives) — tolerates 2 concurrent failures. With sdc faulted, only 1 more failure is tolerable. Zero redundancy margin until replaced."

2. **Check remaining drives for SMART warnings.** Run SMART health on every drive still online in the degraded vdev. A second pre-failure drive means urgency is much higher. Report results per-drive.

3. **Reference prior memory for the faulted drive.** If para-memory-files contain SMART history for this drive (e.g., prior reallocated sectors, temperature trends), connect the dots — "sdc had 2 reallocated sectors noted March 28, predicted failure now materializing."

4. **Specify replacement drive in the approval.** Include the failed drive's model, capacity, interface, and serial from SMART or system inventory. The approval payload must tell BuildOps exactly what to procure or use as replacement.

5. **Post-replacement verification plan.** The approval must include: resilver monitoring (`zpool status`), full pool scrub after resilver completes, and SMART check on all drives post-scrub.
...
Fields to include: issue title, severity change, redundancy tolerance, risk window (data loss if another drive fails), and immediate action needed.
...
6. **Save findings to memory.** After the investigation, record: drive failure, serial, pool impact, SMART history connection, and actions taken. This builds institutional knowledge for future drive events.
