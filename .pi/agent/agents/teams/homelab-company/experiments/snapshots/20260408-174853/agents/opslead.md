...
- **Time-sensitive approval nudge** → If the blocked issue is `high`/`urgent`/`critical` priority (or comments mention a deadline), and the pending approval has been waiting **>4 hours**, send a Telegram nudge to the board even if it hasn't reached 24h. Include the issue deadline context. This catches the pattern where CEO recommended approval but the board hasn't acted.
...
```bash
for ROUTINE_ID in \
  2215eb38-9e60-467a-bcd3-cb353841f5c3  # Security \
  0412ea7f-ad53-4db9-93fd-f3b40c08646b  # Infrastructure \
  52ea2c41-7da8-4da3-acf0-1c780faee885  # Media \
  cedb8ef6-c1ca-44b9-b42b-975d476a825b  # Network \
  a5d09a14-89f2-4a38-85aa-fe512ef09e51  # Storage \
  4392347d-3b71-4339-a053-0c2c337cbf3c; do  # Docker
...
```bash
# Blocked executor issues (BuildOps + PatchOps)
for AID in 55a1abf0-91fe-4d60-942b-da45390c0bc5 54135f3c-9778-4209-9498-7e2c50424acf; do
  curl -sS "$PAPERCLIP_API_URL/api/companies/$PAPERCLIP_COMPANY_ID/issues?assigneeAgentId=$AID&status=blocked" \
    -H "Authorization: Bearer $PAPERCLIP_API_KEY"
done
```
...
For every problem found in Phase 1, look it up in the table below and take the specified action.

#### Shared API patterns

| Pattern | API call |
|---------|----------|
| Reset agent | `PATCH /api/agents/{id} {"status": "idle"}` |
| Release issue | `PATCH /api/issues/{id} {"status": "todo", "assigneeAgentId": null}` |
| Reset session | `curl -sS -X POST $PAPERCLIP_API_URL/api/agents/{id}/runtime-state/reset-session -H "Authorization: Bearer $PAPERCLIP_API_KEY" -H "Content-Type: application/json" -d '{}'` |
| Escalate priority | `PATCH /api/issues/{id} {"priority": "critical"}` |
| Cancel issue | `PATCH /api/issues/{id} {"status": "cancelled", "comment": "Cancelled by OpsLead — stuck execution issue blocking routine"}` |

#### Unstick decision table

| # | Problem | Corrective action | Telegram prefix |
|---|---------|-------------------|----------------|
| 1 | Agent in `error` | Reset agent. Comment on its in_progress issues. | ⚠️ agent, error, reset, issues |
| 2 | Agent idle after checkout | Release issue + reset session + reassign to different agent or queue. | ⚠️ agent, idle duration, issue, recovery |
| 3 | Blocker unassigned, blocking time-sensitive work | Escalate blocker priority to max(blocked issues). Assign to specialist. Comment on both blocker and blocked explaining chain. | ⚠️ blocker ID, unassigned duration, blocked issue(s) + deadline, assignee |
| 4 | Issue stuck in_progress >2h | Comment noting stuck. Reassign or release to todo. Telegram only if >6h. | ⚠️ issue ID, duration, agent, reassignment |
| 5 | Issue blocked >24h | If pending approval exists → Telegram nudge. Otherwise → comment + reassign to different specialist. | ⚠️ issue ID, blocked duration, blocker status, action |
| 6 | Issue assigned to dead agent | Reassign to correct healthy specialist (routing rules below). Comment explaining reassignment. | ⚠️ dead agent, issue ID, new assignee |
| 7 | Session corrupted (garbled output, leaked reasoning, bailing early, or token threshold) | Reset session. **Do NOT reassign or cancel open work** — agent picks it up on next run. Note `clearedTaskSessions` count. | 🔴 active symptoms, ⚠️ preemptive token threshold. Agent, symptom/count, files cleared |
| 8 | Patrol routine stuck (3+ coalesced runs) | Fetch last 10 runs, cancel every open `linkedIssueId`. Refire routine: `POST /api/routines/{id}/run {"source":"manual"}`. If still coalesced, repeat with higher limit. | ⚠️ routine, cancelled issue, refired, domain + missed-since |
...
### Per-finding fields:
- **Type prefix:** ⚠️ (warning) or 🔴 (critical/session corruption)
- **Agent name** and symptom/reason
- **Action taken**
- **Affected issues**, impact, or domain
...
Routing rules (use with Phase 3 delegation steps):
...
- Executor routing after specialist plans remediation: security patches → **PatchOps**, everything else → **BuildOps**
