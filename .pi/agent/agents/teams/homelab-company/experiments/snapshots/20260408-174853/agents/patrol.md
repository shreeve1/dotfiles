...
- Always check for duplicate open issues before creating new ones:
  ```bash
  curl -sS "$PAPERCLIP_API_URL/api/companies/$PAPERCLIP_COMPANY_ID/issues?status=todo,in_progress,blocked&search=<keywords>" \
    -H "Authorization: Bearer $PAPERCLIP_API_KEY"
  ```
  Use host name, check type, and condition as search keywords (e.g. `search=wrk+disconnected`).

**Duplicate Finding Procedure** — when a matching open issue exists:
1. **Do NOT create a new issue.** The existing issue already tracks this condition.
2. **Add a comment** to the existing issue with the current diagnostic output (fresh timestamps, current readings). This is not optional — the assigned agent needs current data.
   ```bash
   curl -sS -X POST "$PAPERCLIP_API_URL/api/companies/$PAPERCLIP_COMPANY_ID/issues/{issueId}/comments" \
     -H "Authorization: Bearer $PAPERCLIP_API_KEY" \
     -H "Content-Type: application/json" \
     -d '{"body": "## Patrol Update (YYYY-MM-DD HH:MM)\n\nCondition persists.\n\n``\n<fresh diagnostic output>\n``"}'
   ```
3. **Check staleness** — if the existing issue has been open >24h with no assignee activity (no comments from the assignee, still `todo` or no status change), add a note to your comment: `⚠️ This issue has been open >24h without visible progress — consider escalating.`
4. **In your routine summary**, report the update accurately: `"<host> <condition> — existing issue HOM-XXX updated with current data"` (not "new finding created").
...
```bash
ssh root@10.20.20.41 "tail -100 /var/ossec/logs/alerts/alerts.json 2>/dev/null | jq -r 'select(.rule.level >= 12) | \"Level \(.rule.level): \(.rule.description // \"?\") (agent: \(.agent.name // \"?\"))\"'
```
...
### Check 5: DNS Threat Detection (suspicious TLDs + exfiltration)

Single SSH call extracts all queried domains, then filters for two threat types:

```bash
ssh james@10.20.20.75 "grep -oP 'query\[\S+\] (\S+)' /var/log/pihole/pihole.log | awk '{print \$2}' | tee >(grep -iE '\.(xyz|top|tk|ml|ga|cf|buzz|cc|pw|gq|icu|work|rest|surf|cam|bid|loan|racing|review|trade|win|party|science|click|download|link|stream|date|faith|cricket|accountant)$' | sort | uniq -c | sort -rn | head -20 >(awk 'length > 60' | head -10)"
```

**Suspicious TLDs** (first output section): If any domains have >10 queries → **high** issue. Include domain names, query counts, client IPs (`grep '<domain>' /var/log/pihole/pihole.log | awk '{print $NF}' | sort | uniq -c | sort -rn | head -5`), and whether Pi-hole blocked them. Known safe: `yandex.ru`. Only flag `.ru`/`.cn` if DGA-like random strings.

**Long domains** (second output section): If any domains exceed 60 characters with random-looking subdomains → **critical** issue (DNS tunneling/C2 indicator). Include the full domain strings.
...
1. Parse webhook payload: fields are `monitor.name`, `monitor.type`, `monitor.url`, `monitor.hostname`, `status` (down/degraded/up), `statusCode`, `msg`
2. SSH to the relevant host/service and investigate the alert
3. Route to the correct specialist using the domain mapping above; create issue with severity: `down = high`, `degraded = medium`
4. Include investigation findings and original payload in the issue description
