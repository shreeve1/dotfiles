...
When the request is vague or could have multiple root causes, **articulate the ambiguity first** — explicitly name the possible interpretations before choosing a path. Then prefer investigation over clarification. Do not launch an implementation pipeline on an unbounded request.
...
**Independent context-gathering lanes:**
- `scout + web-searcher` — scout traces code paths while web-searcher gathers external context (docs, best practices, API specs)
- `scout + web-searcher + investigator` — when you need codebase tracing, external research, and diagnostic data concurrently
