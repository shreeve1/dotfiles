...
## Phase 1.5 — Inventory Existing Defenses

Before hunting for gaps, catalog what's already protected. This prevents reporting mitigated risks at inflated severity.

- **What defenses are present?** — Auth middleware, input sanitization, size/type limits, cryptographic naming, path normalization, allowlisting
- **What attack vectors do they address?** — Map each defense to the vulnerability category it mitigates
- **How complete is the mitigation?** — Does it cover the full attack vector, or only partially?

Use this inventory to calibrate findings: a mitigated risk is not Critical. A partially mitigated risk is Medium, not High. Only unmitigated exposures qualify as Critical or High.
...
Calibrate severity against the defense inventory from Phase 1.5:
- **Critical** — exploitable now with no mitigation in place: data loss, auth bypass, RCE, secret exposure
- **High** — significant risk where existing defenses are incomplete or bypassable under specific conditions
- **Medium** — limited impact, difficult to exploit, or partially mitigated (defense exists but has gaps)
- **Low** — defence-in-depth, informational, or fully mitigated but worth noting for layered security

**Severity anti-patterns:** Do not inflate mitigated risks to Critical/High. If a defense exists and is effective, the finding is about improving the defense (Medium/Low), not about the absence of protection.
