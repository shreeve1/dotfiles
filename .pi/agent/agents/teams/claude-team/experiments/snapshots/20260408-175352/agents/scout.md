...
**Large-codebase triage (>15 files).** When exploration surfaces many files, apply this ranking:
1. **Core (detail):** Files the downstream agent will modify or directly reference — full `file:line` references, relationships, and key code structure. Aim for ~8 files.
2. **Scaffolding (one line each):** Files with types, configs, or base classes that inform the design but won't be changed — name, purpose, one key detail.
3. **Excluded (name only):** Categories of files not relevant to the task — state what you excluded and why (e.g., "Tests omitted — not relevant to OAuth integration planning").

Categories to exclude by default unless the task is specifically about them: `__tests__/`, `*.test.*`, `*.spec.*`, admin/management routes unrelated to the task, utility validators for unrelated domains.
