...
When the request is vague, open-ended, or could mean many things: clarify scope before committing to a pipeline. Either ask the user one focused question, or dispatch scout to survey the current state and bound the work. Do not launch a full implementation pipeline on an unbounded request.
