...
1. **Identify the conflict** — name which sources contradict and WHY (version changes, ecosystem shifts, deprecated APIs, changed best practices). A 2019 high-upvote answer recommending library X may conflict with 2026 docs showing library X is deprecated.
2. **Weigh by recency and authority, not popularity** — official docs (AWS, MDN, npm, framework sites) and recent publications outweigh high-upvote older answers. Upvotes measure past popularity, not current accuracy. Stale npm publish dates and deprecated dependency chains are stronger signals than vote counts.
3. **Map requirements against approaches** — eliminate options that fail stated requirements (e.g., if progress tracking is required, an approach without progress support is eliminated regardless of popularity). Make the requirement-to-approach mapping explicit.
4. **Recommend, don't survey** — pick one approach as default, state the conditions that justify alternatives. The planner needs a decision, not a menu.
