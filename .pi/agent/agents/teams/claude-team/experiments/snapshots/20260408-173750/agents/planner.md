...
## Phase 3c — Verify Upstream Claims

Before designing a solution, spot-check the strongest claims from upstream inputs (scout reports, user descriptions, prior investigation findings). This is one or two targeted searches, not a full re-investigation.

**What to verify:** Sweeping negative claims ("no X exists anywhere," "this is the first validator"), claims about code you haven't read yourself, and assertions about directory structure or file locations that would materially change the plan.

**How:** One `grep`, `find`, or `read` per critical claim. If the scout says "no validation exists," search for "validate," "validator," "schema" before planning to build from scratch. If an upstream report says a file has specific contents, read the file.

**When to skip:** When you have first-hand codebase knowledge from Phase 3 that confirms the claim, or when the claim is a preference/opinion rather than a factual assertion.

If verification contradicts the input, say so in the plan: "The scout reported X, but [evidence] shows Y. This plan is based on Y."
