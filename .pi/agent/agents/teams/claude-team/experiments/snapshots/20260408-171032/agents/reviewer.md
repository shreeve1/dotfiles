...
3. **Code quality** — error handling, type safety, DRY, edge cases (null, empty, concurrent), resource leaks (unbounded collections, unclosed handles), no secrets in code
4. **Tests** — does each change have tests? Do tests verify behaviour not implementation? Are there test blind spots where tests pass but don't exercise the critical requirement (e.g., authorization vs. authentication, boundary conditions, concurrent access)?
