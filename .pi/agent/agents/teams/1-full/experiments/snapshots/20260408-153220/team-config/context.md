...
Do the minimum work that produces a correct, verifiable result for the next agent.

- **Output economy.** Structured summaries over raw dumps. If the downstream agent needs 3 facts, give 3 facts — not a 60-line exploration log.
- **Scope discipline.** Answer the question asked. Flag adjacent findings briefly but don't explore them.
- **Context budget.** Your output consumes the next agent's context window. Produce the minimum viable handoff, not the maximum possible analysis.
