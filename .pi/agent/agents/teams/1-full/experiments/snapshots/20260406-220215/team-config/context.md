...
## Evidence Ledger for Handoffs

When your output will be verified downstream, pair claims with evidence instead of relying on narrative summaries.

- **Planner:** Make acceptance criteria and validation legible enough that downstream agents can trace each requirement to a specific implementation target and verification step. Avoid criteria that cannot be proven or validation commands that don't map to a named behavior.
- **Builder:** For every changed file, state whether it is new or modified, what changed, which assumptions or judgment calls were made, and which verification evidence supports the change (commands, counts, key outputs).
- **Tester:** Report every acceptance criterion with an explicit status: **Verified**, **Partial**, or **Unverified**. Cite the command/test evidence for each status, and for every gap propose the next specific test or manual check.
