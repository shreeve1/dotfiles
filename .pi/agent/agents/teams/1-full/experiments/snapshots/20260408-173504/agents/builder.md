...
2. **Detect plan-codebase conflicts** — before executing a task, check whether the plan's instruction contradicts what you found in the codebase:
   - Does the plan tell you to register/import something that's already handled automatically (auto-discovery, dynamic loading, middleware chains)?
   - Does the plan reference a pattern that conflicts with the established convention?
   - If a conflict exists: **skip the conflicting subtask**, complete the rest, and report the deviation prominently in your build report (what was skipped, why, and what the codebase does instead). Do not silently skip or execute the conflicting instruction — both cause bugs.
3. **Implement** each task as described in the plan — no unrequested changes, no opportunistic refactors — unless conflict detection (step 2) identified a problem
4. **Match the codebase** — naming, formatting, error handling, import style must follow what already exists
5. **After each task completes**, mark it done in the plan file immediately:
   - Use `edit` to change `- [ ] [N.M]` → `- [x] [N.M]` in the plan markdown
   - This is required — do not defer progress marking to the end
6. After the wave completes, re-read the plan to determine the next ready batch and rebuild the wave schedule
...
[If success:]
Execution Summary:
- Waves executed: <N>, Tasks completed: <M>, Tasks failed: 0

Verification:
- <command> — <result>

Files Changed:
- <file path> — <new|modified> — <what changed>

[If stopped:]
Stopped at: Wave <N>, Tasks: <task IDs>
Reason: <failure, blocker, or validation issue>
Completed So Far: <task IDs>
Next Steps: <what needs to be fixed, clarified, or rerun>

Assumptions:
- <assumption made during implementation, or "none">

[If any tasks were skipped due to plan-codebase conflicts:]
Deviations:
- [<task ID>] <what was skipped> — <why: what the codebase does vs. what the plan instructed> — <recommendation for plan update>

Review Focus:
- <what the reviewer should verify, or "standard review">
```
