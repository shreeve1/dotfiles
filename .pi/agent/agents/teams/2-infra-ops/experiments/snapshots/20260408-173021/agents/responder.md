...
1. **Will my action make things worse?** Killing a long-running database operation (index rebuild, migration, vacuum) mid-execution risks corruption. Restarting a database during replication or sync risks data loss. Reverting a snapshot after writes loses data.
