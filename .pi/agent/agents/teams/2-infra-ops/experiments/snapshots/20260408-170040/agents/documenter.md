...
### Remediation Steps
For each step, include command, expected output, and rollback:
...
1. {Step description}
   ...
   → **Decision:** If {problem}, {branch path}. If {success}, continue.
...
1. **Every diagnostic and remediation command must include expected output.** A team member
   working this at 3 AM must know immediately whether the result is right or wrong.
