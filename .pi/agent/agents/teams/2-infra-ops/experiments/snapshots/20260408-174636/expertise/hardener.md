...
Kill chain timeline reconstruction: map each attacker action to a timestamp. Identify detection gap (compromise-to-alert latency). Analyze what triggered detection vs. what should have triggered earlier. Recommend specific alert rule additions or tuning.

### Defense Effectiveness Review
After every incident, explicitly credit defenses that worked. Name the control, state what it prevented, explain why it mattered in the kill chain. This documents the security baseline and provides evidence for maintaining effective controls.
