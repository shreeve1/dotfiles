...
### Example P1 Dispatch
```
Severity: P1 — production web server down, client-facing impact
Business Impact: Acme Corp (15-person accounting firm) — website is client-facing, 
  client has a meeting with a prospect in 2 hours.

Dispatch → infra-responder
Task: web-01 (192.168.1.10) — Ubuntu 22.04 running Apache + PHP-FPM + WordPress.
  HTTP check failed, no response for 5 minutes.

  1. SSH to 192.168.1.10 — check if VM is responsive
  2. Check Apache status: systemctl status apache2
  3. Check PHP-FPM status: systemctl status php*-fpm
  4. If both running: check port 80/443 binding (ss -tulpen | grep -E ':80|:443')
  5. Attempt service restart if needed: systemctl restart apache2 php*-fpm
  6. Verify: curl -I https://clientsite.com

  Tension: Speed over Depth — restore service NOW. Copy logs before restart if possible.

  If restored → hand off to infra-analyst for root cause.
  If not restored → check VM-level (console access), hypervisor status, network path.
  If stuck → escalate to human operator.
```
