...
When flagging findings during discovery, classify every concern using this framework:

| Severity | Definition | Examples |
|----------|-----------|----------|
| **Critical** | Active exploitation risk or data loss imminent | Internet-exposed services with no auth, encrypted data with no backup, active compromise indicators |
| **High** | Significant security or reliability risk, likely to cause failure or breach | Services listening on 0.0.0.0 with no access controls, unattended auto-updates in production with no approval, backup failures beyond 24h |
| **Medium** | Degraded posture or approaching risk; should be addressed within a maintenance cycle | Stale patches (uptime >90 days), stale AV definitions, short DHCP leases, missing redundancy, no backup verification |
| **Low** | Improvement opportunity; capture for future planning | Non-standard naming, duplicate tooling, short lease times, cosmetic config drift |

### Classification Rules
1. **Split concerns into Security and Operational buckets.** Security concerns affect attack surface or data protection. Operational concerns affect reliability, recoverability, or maintainability.
2. **Assign severity per the table above.** If a finding fits multiple severities, use the higher one.
3. **Include a one-line justification** for each High or Critical finding so the reader understands why it earned that rating.
4. **Cross-reference dependencies.** If a concern affects a dependency chain (e.g., "Pi-hole DNS for all hosts"), note the blast radius.
5. **Flag patch currency.** Any host with uptime >90 days should be flagged as Medium operational concern (likely missing OS patches).

## Dependency Chain Mapping

When building the service map, do not just list services per host. Trace and document dependency chains — the paths where one service depends on another across hosts. Every discovered service should be connected to at least one chain or explicitly noted as standalone.

### Chain Tracing Method

1. **Identify consumers and providers.** For each service, ask: what does it talk to? What talks to it? A database is a provider; the app connecting to it is a consumer. DNS and DHCP are providers consumed by every host.

2. **Follow the data.** Trace where data flows: application → database, application → external API (via VPN/tunnel), backup agent → backup target, monitoring agent → monitoring server.

3. **Identify shared infrastructure dependencies.** These are consumed by multiple hosts and are often the highest blast radius: AD/DNS/DHCP (every domain-joined host), DNS filtering (pihole — every host using it for resolution), backup targets (all hosts writing backups there), monitoring (all agents reporting there).

4. **Classify chains by criticality.** A chain is **critical path** if its failure stops business operations (e.g., Dentrix connector → VPN → cloud → database). A chain is **supporting** if its failure degrades but does not stop operations (e.g., monitoring, backups).
...
### Common Chain Patterns to Check

- **Application stack:** web/proxy → app server → database
- **Authentication/identity:** AD → DNS → all domain-joined hosts
- **External connectivity:** app → VPN tunnel → cloud service
- **Backup chain:** host backup agent → backup target → (offsite replication?)
- **DNS resolution:** filtering DNS → all hosts → upstream resolver
- **Monitoring:** agents on each host → monitoring server
- **DHCP:** DHCP server → all dynamic-IP hosts

### Blast Radius Note

When a dependency serves multiple consumers (e.g., DNS, backup target, AD), explicitly state the blast radius: "If backup-01 fails, dc-01 backups and db-01 pg_dump both stop." This is the most important output of dependency mapping — it tells the team what breaks when something goes down.
...
### Dependency Chains
{traced chains with path, criticality, and blast radius — per Dependency Chain Mapping section above}
...
### Cross-Host Analysis
Before finalizing concerns, compare findings across all hosts. Check for:
- **Duplicate services/tooling:** Same service running on multiple hosts (e.g., two monitoring instances, two DNS resolvers) — flag as operational concern with recommendation to consolidate or document why both exist.
- **Shared patch debt:** Multiple hosts with uptime >90 days indicates systemic patch currency problem, not isolated per-host issues.
- **Backup coverage gaps:** List which hosts have verified backups vs. no backup/failed backup/stale backup in a single comparison table.
- **Single points of failure:** Services or hosts that have no redundancy and serve multiple consumers.
- **Configuration drift patterns:** Same service configured differently across hosts (e.g., one firewall ON, another OFF).

Present as a brief structured comparison, not per-host narrative. This ensures patterns that only emerge when viewing hosts together are caught systematically.

### Security Concerns
{findings that affect attack surface or data protection, each rated Critical/High/Medium/Low with justification}

### Operational Concerns
{findings that affect reliability, recoverability, or maintainability, each rated Critical/High/Medium/Low with justification}
