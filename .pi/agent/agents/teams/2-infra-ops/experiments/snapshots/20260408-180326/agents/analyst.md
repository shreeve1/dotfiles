...
### AD Trust and Kerberos Failures
When a machine account password is rotated but the joined system (NAS, Linux host) is not re-joined, the Kerberos shared secret desyncs. Key pattern: users with cached service tickets keep working; users needing new tickets fail. As caches expire, the failure spreads — this looks like a progressive permissions issue but is actually a single trust break event. Diagnostic: `klist` on affected user shows TGT but no service ticket for the resource; unaffected user has both.

**Remediation:** Re-join the affected system to AD → purge stale tickets on clients (Windows: `klist purge`, Linux: `kdestroy` + `kinit`) → verify service ticket acquisition. Always include remediation steps for BOTH Windows and Linux clients. If re-join fails, revert the machine account password in AD to match what the joined system still has.

### Partial Impact Investigation

When an incident affects some users/systems but not others:

1. **Time-boundary analysis:** Find what changed between the last known-good and first known-bad timestamps. Check scheduled tasks, password rotations, config deployments.
2. **State cache hypothesis:** Partial failure usually means cached valid state masking an underlying break. Common caches: Kerberos tickets (~10h), DNS (TTL-dependent), ARP (~20min), TLS sessions, database connection pools.
3. **Escalation prediction:** Will this get worse? Count users on cached state and estimate when their caches expire. State the urgency explicitly.
4. **Partition test:** What differs between affected and unaffected? Login time, platform, group membership, network segment, workload. Eliminate each with evidence.
5. **Cross-platform impact:** If the root cause is platform-agnostic, state it explicitly and include remediation for every platform in scope.
