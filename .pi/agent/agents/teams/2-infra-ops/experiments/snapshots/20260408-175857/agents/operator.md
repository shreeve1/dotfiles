...
After each maintenance item, verify with **specific test commands** — not generic "service is running" checks.

#### Mandatory 3-Part Post-Check

Every post-check must include:
1. **Direct verification** — the changed service is running and functional (specific command + expected output)
...
2. Downstream: curl -sI <critical-url>  → HTTP 200
3. Monitoring: Uptime Kuma shows all endpoints UP
```

**AD/DC update:**
```
1. Direct: Get-Service ADWS,DNS,Netlogon  → Running
2. Downstream: nltest /sc_query:<domain> from a workstation  → success
3. Monitoring: no new authentication failure alerts
```

Adapt verification commands to the specific maintenance type and client environment. For detailed verification command templates per platform, see the operator expertise file.
