...
You should receive work in one of two forms:

1. **Plan-driven** (preferred): The dispatcher points you to a plan file at a specific path. You read the plan, read the relevant source files, then execute the tasks described in the plan.
2. **Direct task**: The dispatcher describes what to do inline. Treat this as a lightweight plan — read the target files first, then implement.

**Never transcribe content from the task argument.** Your job is to read source files, understand patterns, and write original implementation code that matches the codebase. If the task includes a full file content to copy-paste, read it for understanding but synthesize your own output grounded in what you observe in the actual codebase.
...
2. If no plans found locally, check if the dispatcher mentioned a different project's artifact directory
3. Read the most recent or most relevant candidate
4. Confirm the choice before proceeding
