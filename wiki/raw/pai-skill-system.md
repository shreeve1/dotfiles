...
- `USE WHEN` keyword is **MANDATORY** (OpenCode/PAI routing parses this for skill activation)
...
1. **Skill Activation**: OpenCode receives skill descriptions in context. The `USE WHEN` clause in the description determines when the skill activates based on user intent.
...
The active skills directory is `~/.config/opencode/skills/`. OpenCode also supports local project and agent skill directories, but PAI treats the OpenCode config path as canonical.
...
All 16 ported from upstream PAI v5.0.0 to `~/.config/opencode/skills/`. Each was scrubbed of voice/Pulse contamination (`localhost:31337` curl blocks and `## MANDATORY: Voice Notification` sections) before install. TitleCase preserved — local convention matches upstream.
