...
> **OpenCode status, 2026-05-10:** This document is primarily the legacy OpenCode hook reference. The active OpenCode runtime uses plugins under `~/.config/opencode/plugins/` and memory under `~/.pai/memory/`. See `MEMORYSYSTEM.md` for the current OpenCode memory writers and the gap analysis for the not-yet-implemented memory-ingest plugin.

**Event-Driven Automation Infrastructure**

**Location:** `~/.config/opencode/plugins/`
**Configuration:** `~/.config/opencode/opencode.json`
**Status:** Legacy OpenCode reference; OpenCode uses `~/.config/opencode/plugins/`

---

## Overview

The PAI hook system is an event-driven automation infrastructure built on OpenCode's native hook support. Hooks are executable scripts (TypeScript/Python) that run automatically in response to specific events during OpenCode sessions.
...
**Key Principle:** Hooks run asynchronously and fail gracefully. They enhance the user experience but never block OpenCode's core functionality.
...
Hooks have access to all environment variables from `~/.config/opencode/opencode.json` `"env"` section:
...
```json
{
  "env": {
    ...
    "PAI_RUNTIME_HOME": "/home/james/.pai",
...
**Key Variables:**
- `PAI_RUNTIME_HOME` - PAI runtime directory (typically `~/.pai`)
...
**Key Principle:** Hooks must never block OpenCode. Always exit quickly, use background processes for slow work.
...
**Why:** If hooks crash, OpenCode may freeze. Always exit cleanly.
...
**Location:** `~/.config/opencode/plugins/my-custom-hook.ts`
...
```bash
chmod +x ~/.config/opencode/plugins/my-custom-hook.ts
```
...
```bash
# Test hook directly
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","hook_event_name":"Stop"}' | bun ~/.config/opencode/plugins/my-custom-hook.ts
```
...
**Check:**
1. Is hook script executable? `chmod +x ~/.config/opencode/plugins/my-hook.ts`
2. Is path correct in settings.json? Use `${PAI_DIR}/hooks/...`
3. Is settings.json valid JSON? `jq . ~/.config/opencode/opencode.json`
4. Did you restart OpenCode after editing settings.json?

**Debug:**
```bash
# Test hook directly
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","hook_event_name":"Stop"}' | bun ~/.config/opencode/plugins/my-hook.ts

# Check hook logs (stderr output)
tail -f ~/.config/opencode/plugins/debug.log  # If you add logging
```
...
**Resolution:** Fixed in OpenCode updates. The Stop hooks now fires reliably. The unified orchestrator pattern (`Stop hooks.hook.ts` delegating to `handlers/`) was implemented in part to work around this — and remains the production architecture.
...
tail -f ~/.config/opencode/plugins/subagent-stop-debug.log
```
...
**Root Cause:** OpenCode transcripts use `type: "user"` but hooks were checking for `type: "human"`.
...
grep '"type":"user"' ~/.pai/projects/-Users-username--claude/*.jsonl | head -1 | jq '.type'
...
bun ~/.config/opencode/plugins/LoadContext.hook.ts
...
- Permission denied → `chmod +x ~/.config/opencode/plugins/LoadContext.hook.ts`
...
KEY FILES:
~/.config/opencode/opencode.json              Hook configuration
~/.config/opencode/plugins/                     Hook scripts (22 files)
~/.config/opencode/plugins/handlers/            Handler modules (6 files)
~/.config/opencode/plugins/lib/                 Shared libraries (13 files)
~/.config/opencode/plugins/lib/learning-utils.ts Learning categorization
~/.config/opencode/plugins/lib/time.ts          PST timestamp utilities
~/.config/opencode/plugins/lib/event-types.ts   Typed event definitions (22 interfaces)
~/.config/opencode/plugins/lib/event-emitter.ts appendEvent() → events.jsonl
...
The PAI brain at `~/.pai/PAI/` is shared between OpenCode and OpenCode. Hooks above are all OpenCode-plugin-native (`UserPromptSubmit`, `PostToolUse`, `Stop`, etc.). OpenCode has its own plugin event surface and a smaller subset of upstream hooks have been ported as OpenCode plugins under `~/.config/opencode/plugins/`.
...
|---|---|---|---|
| `CheckpointPerISC.hook.ts` | `pai-checkpoint-per-isc` | `tool.execute.after` (write/edit on ISA.md) | Auto-commits opted-in repos on `[ ]→[x]` ISC transitions. Allowlist at `~/.pai/checkpoint-repos.txt`. |
| `ISASync.hook.ts` | `pai-isa-sync` | `tool.execute.after` (write/edit on ISA.md) | Read-only mirror to `~/.pai/memory/STATE/work.json`. Pulse syndication stripped. |
| `ContainmentGuard.hook.ts` + `containment-zones.ts` | `pai-containment-guard` | `tool.execute.before` (write/edit) | Inlined zones (`PAI/USER`, `PAI/MEMORY`, `MEMORY`, `settings.local.json`). Patterns user-supplied at `~/.pai/PAI/USER/containment-patterns.txt`. |
| `ConfigAudit.hook.ts` | `pai-config-audit` | `tool.execute.after` (write/edit on opencode.json) | Append-only sha256 + size audit log to `~/.pai/memory/OBSERVABILITY/config-changes.jsonl`. |
...
- `pai-reflection-loop` — `session.idle` ensures `~/.pai/memory/LEARNING/REFLECTIONS/` exists, rotates the JSONL when it crosses 5 MiB, and appends a session-idle marker.
