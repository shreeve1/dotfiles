...
| **Extended (E2)** | <3min | **≥8 (soft, v6.4.0)** | **≥2** | **≥1** | Quality must be extraordinary |
...
**1. Thinking-capability floor (HARD, v6.1.0 — CANONICAL ENUMERATION + `OTHER:` escape as of v6.4.0).** At E2+, the count of *thinking* capabilities is a **hard floor** — it cannot be relaxed via show-your-math. Difficult work earns thinking depth, full stop.

**The thinking-capability vocabulary is a CANONICAL ENUMERATION with an `OTHER:` escape (v6.4.0 LOOSENING).** Selection should come verbatim from this list — the same names that appear in `capabilities.md` § Thinking & Analysis Capabilities. Generic filler labels ("decomposition", "edge-case enumeration", "tradeoff analysis", "deep reasoning", "structured thinking") are **PHANTOMS** and do NOT contribute to the tier floor. However, when a genuine thinking move does not match any canonical name, you MAY name it as `🏹 OTHER: <specific-name> → <PHASE> | <one-line justification of why no canonical name fits>`. `OTHER:` entries count toward the thinking floor at most **once per tier** (E2 ≤1, E3 ≤1, E4 ≤2, E5 ≤2). Overuse of `OTHER:` is a soft audit signal — review whether the missing name belongs in the canonical list and bump the minor version if so.

The canonical list (verbatim names — copy/paste into `🏹 CAPABILITIES SELECTED`; `OTHER:` escape available within per-tier cap):
...
If a name is not in this list verbatim AND not formatted as `OTHER: <name>` with justification, it is a phantom and is rejected by the audit gate. New thinking capabilities are added by editing `capabilities.md` and bumping the Algorithm minor version — the `OTHER:` escape is a per-run pressure-relief valve, not a substitute for canonicalizing recurring patterns.

**Capability-Name Audit Gate (v6.4.0 — fires at OBSERVE→THINK boundary):** before printing `🏹 CAPABILITIES SELECTED`, verify each thinking name either (a) appears verbatim in the canonical list above, or (b) uses the `OTHER: <name>` form with a one-line justification and stays within the per-tier `OTHER:` cap. Names that are paraphrases of canonical entries (e.g., "First-principles decomposition" when `FirstPrinciples` exists) are still phantoms — use the canonical name. Example correct: `🏹 **FirstPrinciples** → THINK | …`. Example correct (escape): `🏹 **OTHER: ScopeNegotiation** → PLAN | no canonical name covers cross-team scope haggling`. Example REJECTED: `🏹 First-principles decomposition → THINK | …` (paraphrase of canonical name).
...
### Mode Classification (v6.3.0, unchanged in v6.4.0)

**Mode and tier are decided by the OpenCode mode router before execution, not by the executor.** The active OpenCode plugin runs on top-level prompts, calls the configured inference surface when needed, and writes a single mode-routing context block:
...
**Tier floor:** the granularity rule produces a natural N. At E2+, that N must meet the tier ISC floor (E2 ≥8 soft as of v6.4.0, E3 ≥32, E4 ≥128, E5 ≥256). E1 has no floor — fast-path stays fast.
...
3. For task ISAs the skill creates `~/.pai/memory/WORK/{slug}/`; for project ISAs the skill reads existing `<project>/ISA.md` if present, or seeds it via the Seed workflow
...
**Phase-marker rendering (v6.4.0 LOOSENING):** the `━━━ PHASE_NAME ━━━ N/7` separator bars are MANDATORY at E3+ (they anchor navigation in long outputs) and OPTIONAL at E1/E2. At E1/E2, inline transitions are preferred: name the phase in prose (e.g., "Thinking: …", "Planning: …") and rely on the phase emoji as the visual cue. Phase tracking via ISA frontmatter `phase:` edits remains unchanged at every tier — this loosening is presentation-only.
...
**Set effort level (classifier-driven, v6.3.0 mechanism unchanged in v6.4.0):**
...
**Select capabilities:** Names MUST come verbatim from the canonical enumeration, OR use the `OTHER: <name>` form with justification (within per-tier cap). Phantom names and paraphrases of canonical entries are CRITICAL FAILURE.
...
| **Thinking floor (HARD)** | Thinking-capability count meets tier hard floor. Names from canonical enumeration verbatim, or `OTHER: <name>` within per-tier cap. |
| **Capability-Name Audit (HARD, v6.4.0)** | Each thinking name appears verbatim in canonical enumeration, OR uses `OTHER: <name>` with justification within the per-tier cap. |
...
**🗣️ VOICE (v6.4.0, optional at any tier, encouraged E2+):** 1–2 freeform sentences in the executor's own register — the thinking move you'd make if no template existed. This is the deliberate slack against checklist-shape: a place to name what's actually interesting, weird, or worrying about this specific run. Not a phantom capability — it does NOT count toward the thinking floor. It is texture, not coverage.

`🗣️ VOICE: [1–2 sentences in your own register]`
...
**Every Algorithm run MUST include this exact block as its primary close. A short freeform tail (v6.4.0) is permitted after, under the rules below.**
...
**After this block (v6.4.0 LOOSENING):** an optional short freeform tail is permitted — at most 3 sentences, only when there is something genuine to add (a caveat, a noticed pattern, a one-line offer of follow-up). No new findings, no restating the SUMMARY. If nothing genuine: stop here.
...
- **Thinking floor (HARD)** — meet tier thinking floor (E2 ≥2, E3 ≥4, E4 ≥6, E5 ≥8). Names from v6.4.0 canonical enumeration verbatim, or `OTHER: <name>` with justification within the per-tier cap (E2 ≤1, E3 ≤1, E4 ≤2, E5 ≤2).
...
2. Check the active ISA and todo state for in-flight work
3. Jump directly to current phase

**Cold-start recovery:**
1. Project work: read `<project>/ISA.md`
2. Task work: read ISA from `~/.pai/memory/WORK/`
