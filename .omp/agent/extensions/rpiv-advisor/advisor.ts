...
import { completeSimple, getSupportedEfforts, type Message, type ThinkingLevel } from "@earendil-works/pi-ai";
import {
...
// ---------------------------------------------------------------------------
// System prompt — inlined rather than read from prompts/advisor-system.txt.
// omp's legacy-pi compat relocates extension modules to a temp dir, so
// import.meta.url no longer points at this file's real location and a relative
// readFileSync would ENOENT. Keep this string in sync with prompts/advisor-system.txt.
// ---------------------------------------------------------------------------
...
export function registerAdvisorCommand(pi: ExtensionAPI): void {
	pi.registerCommand("advisor", {
		...
		handler: async (_args, ctx) => {
			...
			if (picked.reasoning) {
				const levels = getSupportedEfforts(picked).includes("xhigh")
