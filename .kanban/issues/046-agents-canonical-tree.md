...
1. `.agents/AGENTS.md` — a merged global guidance file. Content: the Agent Notes from `.claude/CLAUDE.md` (Simplicity First / Surgical Changes / Explain It Simply, plus the graphify pointer), the three always-on rules folded in as sections (ponytail, explain, legacy — they currently live in `.claude/rules/*.md` and would otherwise be silently lost when dsh-cc-skills is removed), and the codex-specific guidance merged from `.codex/AGENTS.md` (client-ops context, Think Before Coding, Goal-Driven Execution) deduped against the above.
...
- [ ] `.agents/AGENTS.md` exists and contains the Agent Notes (Simplicity First / Surgical Changes / Explain It Simply), the three rules as sections, and the merged codex guidance (client-ops context present)
...
`test -f .agents/AGENTS.md && grep -q 'Surgical Changes' .agents/AGENTS.md && find .agents/skills -maxdepth 2 -name SKILL.md | wc -l` (expect 83) and `find .agents -type l | wc -l` (expect 0)
...
- The merge: `.claude/CLAUDE.md` Agent Notes are the base; append the three rule sections (ponytail 10-, explain 20-, legacy 30-); merge in `.codex/AGENTS.md` sections, deduping "Simplicity First" / "Surgical Changes" which appear in both.
