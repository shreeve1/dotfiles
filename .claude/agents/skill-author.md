...
skill_context = {
  ...
  gotchas:          string[]  — critical constraints the user explicitly described
...
5. **Code examples** — Pull real function names, import paths, and patterns from the existing scripts you read. Do not use placeholder paths like `scripts/halo_client.py` if the actual file is `scripts/api/halopsa_client.py`.
...
- Include both the "happy path" and fallback behaviors
...
- Keep the skill focused on one primary action with secondary actions available via menu
...
3. **Build** - Write the SKILL.md with proper frontmatter and step-by-step instructions
