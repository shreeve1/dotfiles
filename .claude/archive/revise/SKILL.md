...
echo "recent plans:"
node "${CLAUDE_SKILL_DIR}/../_shared/list-recent.mjs" .rpiv/artifacts/plans 10
```
...
The revised artifact stays in `.rpiv/artifacts/plans/` for `/implement` to resume.
...
2. **Handle different input scenarios**:
   ...
   **If a REVIEW artifact path is provided**:
   ...
   Example:
   `/revise .rpiv/artifacts/plans/2025-10-16_09-00-00_feature.md "Address the findings from .rpiv/artifacts/reviews/2025-10-16_10-00-00_feature.md by tightening validation in Phase 2 and expanding success criteria."`
   ```
   Wait for user input.

   **If NO plan file provided**, branch on the `recent plans:` listing in the Metadata block:
   - **Empty** — no plans under `.rpiv/artifacts/plans/`; tell the user and suggest running `/plan` first.
   - **Exactly one entry** — confirm with `AskUserQuestion`: question "Revise this plan?", header "Revise", options [{label: "Revise `<filename>` (Recommended)", description: "Update this plan"}, {label: "Pick a different path", description: "Choose another plan file"}].
   - **Two or more entries** — present the top 4 filenames as `AskUserQuestion` options.

   If the user is coming from `/code-review`, also ask which findings should change the plan. Wait for user selection, then re-check for feedback.
...
Use the `AskUserQuestion` tool to confirm before editing. Question: "{Summary of planned modifications}. Proceed with these edits?", header: "Changes", options: [{label: "Proceed (Recommended)", description: "Apply the planned changes to the existing plan"}, {label: "Adjust approach", description: "Modify what will be changed before editing"}, {label: "Show me first", description: "Show the exact text changes before applying"}].
...
1. **Present the changes made**:
   ...
   💬 Follow-up: describe further plan changes in chat — each `/revise` call appends another timestamped Follow-up section, history is preserved.

   **Next step:** `/implement .rpiv/artifacts/plans/{filename}.md Phase {N}` — resume execution at the affected phase (or omit `Phase {N}` to run all phases sequentially).
...
- **Each invocation appends history.** Every `/revise` call adds another timestamped Follow-up section — do not collapse history. Prior phase decisions stay visible.
...
```
User: /revise .rpiv/artifacts/plans/2025-10-16_09-00-00_feature.md - add phase for error handling
Assistant: {Reads plan, researches error handling patterns, updates plan}
```

**Scenario 2: User provides just plan file**
```
User: /revise .rpiv/artifacts/plans/2025-10-16_09-00-00_feature.md
...
```
User: /revise .rpiv/artifacts/reviews/2025-10-16_10-00-00_feature.md
...
User: /revise .rpiv/artifacts/plans/2025-10-16_09-00-00_feature.md "Address the review findings by splitting Phase 2 and adding validation coverage"
Assistant: {Proceeds with update}
```
