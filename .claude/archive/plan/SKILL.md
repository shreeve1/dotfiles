...
1. **Determine input mode**:
   ...
   **Design artifact provided** (path to a `.md` file in `.rpiv/artifacts/designs/`):
   ...
   - Extract: Architecture (the code changes), **`## Slices` (slice boundaries + per-slice Success Criteria — authored by `/design` Step 6.1, verified by slice-verifier at 6.2)**, File Map, Ordering Constraints, Verification Notes, Performance Considerations, Scope
   ...
   **No arguments provided**:
   ...
   `/plan .rpiv/artifacts/designs/2025-01-20_09-30-00_feature.md`

   Run `/design` first to produce the design artifact. There is no standalone path.
   ```
   Then wait for input.
...
No developer question — boundary changes are out of scope for plan. If the developer wants different boundaries, they revisit `/design` and re-decompose. (This guarantees blueprint-equivalent slice-verified atomicity throughout.)
...
1. **Triage Step 4 reviewer findings first** (skip if Step 4 returned no findings):
   ...
   **Code finding caveat**: when a code finding's root cause is in the design's Architecture (not just plan transcription), the patch belongs upstream. Either (a) Edit the design artifact and re-run `/plan`, or (b) apply the fix to the plan's Phase code fence and annotate the resolution with `applied (plan-local; design follow-up: <design path>)`. Option (a) is cleaner; option (b) is acceptable for tactical fixes.

   **Order and batching**: blockers sequentially (resolution may invalidate later rows). Concerns and suggestions: batch up to 4 independent rows per `AskUserQuestion` call. Independent = different files / different intents AND neither recommendation references the other's location; otherwise sequential.
...
3. **Present the plan location** (after triage is complete):
   ...
   💬 Follow-up: describe the change in chat to append a timestamped Follow-up section to this artifact, or use `/revise <plan-path>` for surgical phase edits. Re-run `/plan` for a fresh artifact.
...
- **When to re-invoke instead.** For surgical edits driven by review findings, prefer `/revise <plan-path>`. Re-run `/plan` only when the underlying design changed materially. The previous block's `Next step:` stays valid for the existing plan.
...
2. **Pass-Through, Not Author**:
   ...
   - The only place plan exercises judgment is Step 5 triage of reviewer findings — and even there, design-root-cause findings should route back to `/design`
...
If the design's criteria don't match this shape, the defect lives upstream — return to `/design` to fix, do not patch in plan.
