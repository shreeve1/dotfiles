...
OpenCode is now the active runtime. PAI memory is stored under the tool-neutral runtime root, not under `~/.claude`.
...
- Claude-era automatic hooks such as `RatingCapture`, `WorkCompletionLearning`, `SessionHarvester`, and `LearningPatternSynthesis` are not currently active OpenCode plugins.
...
4. **Update or split `THEHOOKSYSTEM.md`.** It is still primarily a Claude Code hook reference. Keep it as legacy reference or add an OpenCode plugin-system companion doc.
...
The historical PAI memory design lived under `~/.claude/MEMORY/` and relied on Claude Code transcript storage under `~/.claude/projects/`. That architecture is no longer the active OpenCode runtime path.

Historical repo paths under `~/dotfiles/.claude/...` may still exist as source-controlled compatibility assets. They are not the same as runtime reads from `~/.claude/...`.
