#!/usr/bin/env bash
# pi-orch V2 — live-pane orchestration primitive.
...
#
# Usage:
#   v2-pane-orch.sh <workdir> <worker-task-file> <reviewer-task-file>
#
...
#
# Env:
#   PI_V2_WORKER_MODEL   default minimax/MiniMax-M3
#   PI_V2_REVIEWER_MODEL default deepseek/deepseek-v4-flash
#   PI_V2_THINKING       default low
#   PI_V2_WAIT_MS        per-pane cycle wait (default 900000 = 15 min)
#   PI_V2_MAX_CYCLES     default 3
#   PI_V2_KEEP           if set, skip teardown (debug)
#
...
worker_model=${PI_V2_WORKER_MODEL:-minimax/MiniMax-M3}
reviewer_model=${PI_V2_REVIEWER_MODEL:-deepseek/deepseek-v4-flash}
thinking=${PI_V2_THINKING:-low}
wait_ms=${PI_V2_WAIT_MS:-900000}
max_cycles=${PI_V2_MAX_CYCLES:-3}
...
session_dir="$abs_workdir/.pi-v2-sessions"
log="$abs_workdir/.pi-orch-logs/$(date +%Y%m%d-%H%M%S)-v2-pane.log"
...
wait_ready() {
  ...
  local end=$(( $(date +%s%3N) + timeout_ms ))
...
# --no-skills / --no-extensions: child panes are task-focused; no skill or
...
worker_json=$(herdr agent start "worker" --tab "$tab_id" --split right \
...
reviewer_json=$(herdr agent start "reviewer" --tab "$tab_id" --split down \
  --cwd "$abs_workdir" --no-focus -- \
  pi "${common_pi_args[@]}" \
