---
name: rpiv-monitor
description: Diagnose whether a running rpiv-run pipeline (engine=pi or claude) is working or genuinely stalled, and report which step it is on. USE WHEN user says "is my rpiv run stuck", "check the rpiv pipeline", "monitor rpiv-run", or thinks an unattended rpiv-run looks frozen. Read-only — never types into the live pane.
---

# rpiv-monitor

Diagnose a running `rpiv-run` pipeline read-only and report: which step, working vs. stalled, and (if stalled) the likely cause. The driver runs each step in a fresh engine TUI on a **private** tmux socket, so the user's own terminal only shows quiet driver stdout — "looks frozen" is usually just invisibility, not a stall.
...
```bash
ps aux | grep -E '[r]piv-run|[p]i --append|[c]laude --permission' | grep -v grep
```

Take the `rpiv-run` driver PID. Then:
...
**Find the socket from the driver, not `ls -t`.** `ls -t /tmp/rpiv-tmux-*.sock` is unreliable — a stale socket file from a prior day can sort first by mtime while having no live server. Instead, list sessions on each candidate socket and keep the one with a live server:

```bash
for s in /tmp/rpiv-tmux-*.sock; do
  echo "== $s =="; tmux -S "$s" list-sessions 2>&1
done
```

The socket that reports `rpiv-<step>-<n>: 1 windows ...` is the live one. The **step name is in the session name** (`rpiv-research-...` → step = research). Pipeline order: research → design → plan → implement → validate → code-review → commit.

Record: `SOCK`, `S` (session), step, repo cwd, driver PID, start time.
...
```bash
a=$(tmux -S "$SOCK" capture-pane -p -t "$S":0.0 | md5sum)
sleep 6
b=$(tmux -S "$SOCK" capture-pane -p -t "$S":0.0 | md5sum)
...
```bash
tmux -S "$SOCK" capture-pane -p -J -S - -t "$S":0.0 | tail -25
```
...
With the live-log change in `bin/rpiv-run`, the step log is now written **during** the step (~every 5s), so this also works for a live tail:
...
Do not change `bin/rpiv-run` from this skill. If a code change is warranted, hand off to `dev-review-pi` / `code-review` then `/commit`.

## Source of truth

- Driver: `~/dotfiles/bin/rpiv-run` (header comments document engine model, pi readiness-settle, timeouts).
