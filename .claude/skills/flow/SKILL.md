...
```bash
cd ~/my-project
~/.claude/PAI/Tools/ralph-loop.sh --limit 10
```

Next morning:
```bash
~/.claude/PAI/Tools/ralph-loop.sh --review    # review last issue
~/.claude/PAI/Tools/ralph-loop.sh --stale     # check for crashed runs
...
|------|----------|---------|
| `/grill-me` | `~/.claude/skills/grill-me/` | Alignment interview |
| `/to-prd` | `~/.claude/skills/to-prd/` | PRD creation |
| `/to-issues` | `~/.claude/skills/to-issues/` | Vertical slice breakdown |
| `/kanban` | `~/.claude/skills/kanban/` | Local board management |
| `/ralph` | `~/.claude/skills/ralph/` | AFK implementation loop (in Claude Code) |
| `ralph-loop.sh` | `~/.claude/PAI/Tools/ralph-loop.sh` | AFK implementation loop (bash, external) |
| `/tdd` | `~/.claude/skills/tdd/` | Test-driven development |
| `/codex review` | gstack skill | Independent OpenAI review |
| `/improve-codebase-architecture` | `~/.claude/skills/improve-codebase-architecture/` | Deep module analysis |
| `/design-an-interface` | `~/.claude/skills/design-an-interface/` | Compare interface designs |
