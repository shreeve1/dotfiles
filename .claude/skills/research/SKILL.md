...
**Default agent**: `codebase-analyzer` for all codebase questions. This agent has Read, Grep, Glob — it can trace code paths, find patterns, and analyze integration points.
...
2. **Developer checkpoint — grounded questions one at a time:**
   ...
   **Choosing question format:**
   ...
   - **`AskUserQuestion` tool** — when your question has 2-4 concrete options from code analysis (pattern conflicts, integration choices, scope boundaries, priority overrides). CC automatically adds an "Other" free-text choice, so do NOT add your own. Each option needs a short `label` and a `description`. Header ≤12 chars. Example:

     > Use the `AskUserQuestion` tool. question: "Found 2 patterns for retry logic — which is canonical?". header: "Pattern". options: [{label: "Event-sourced (Recommended)", description: "`src/events/orders.ts:45-67` — 3 hooks, matches precedent commit `abc123`"}, {label: "Direct retry loop", description: "`src/services/OrderService.ts:112` — single use, no event traceability"}].

   - **Free-text with ❓ Question: prefix** — when the question is open-ended and options can't be predicted (discovery, "what am I missing?", corrections). Ask it as a plain chat message, not via the tool. Example:
     "❓ Question: `src/events/orders.ts:45-67` has 3 event hooks but no error recovery path. Is there a retry mechanism elsewhere I'm not seeing?"
   ...
   **Anti-pattern** — do NOT dump a verbose paragraph mixing analysis with a trailing question:
   ...
   ✅ Extract the 2 concrete options and call `AskUserQuestion`: question "Where is the prompt bloat landing?", header "Bloat source", options [{label: "Named agents (Recommended)", description: "Skills dispatch `codebase-analyzer` etc. — `prompt_mode: replace`, no parent inheritance"}, {label: "General-purpose", description: "`default-agents.ts:11-28` — `promptMode: append`, inherits full parent prompt"}].

   **Batching**: When you have 2-4 independent questions (answers don't depend on each other), you MAY batch them in a single `AskUserQuestion` call (the tool accepts multiple questions). Keep dependent questions sequential.

   **CRITICAL**: Ask ONE question at a time when questions are dependent. Wait for the answer before asking the next. Lead with your most significant finding.
...
💬 Follow-up: describe the change in chat to append a timestamped Follow-up section to this artifact. Re-run `/research` for a fresh artifact.

**Next step (choose one):**
- `/design .rpiv/artifacts/research/{filename}.md` — iterative design with vertical-slice decomposition (produces design artifact for plan)
- `/blueprint .rpiv/artifacts/research/{filename}.md` — lightweight fast path for smaller tasks; combined design + phased plan in one pass (produces implement-ready plan directly)
...
- **When to re-invoke instead.** If scope changed materially (different feature surface, different research target), re-run `/research` for a fresh artifact. The previous block's `Next step:` stays valid for the existing artifact.

## Important Notes

- **Analysis only**: This skill answers questions. Question formulation is delegated to the scope-tracer subagent at Step 1.
- **Single entry point**: Free-text research prompt. `$ARGUMENTS` carries the input; scope-tracer runs in-band before analysis dispatch.
