...
## Global Profiles vs Project Sessions

**Decision rule:**
- Task requires **saved credentials** (logging in, staying logged in across projects) → use a **global profile** from `~/.playwright-profiles/`
- Task is **project-scoped** (scraping, UI tests with no login) → use a **project session** in `.playwright-sessions/`

### Global Profiles (`~/.playwright-profiles/`)

Credentials shared across all projects and skills. Two types:

**Persistent context** (full browser profile — use for complex auth, sessionStorage):
```bash
# Create/login - open browser, log in, close to save
node -e "
const { chromium } = require('playwright');
chromium.launchPersistentContext(
  require('os').homedir() + '/.playwright-profiles/{name}',
  { headless: false }
).then(ctx => ctx.pages()[0].goto('https://example.com'));
"
```

**Storage state** (cookies + localStorage — use for simple auth, works with CLI flags):
```bash
# Create/login
npx playwright open --headed \
  --storage-state=$HOME/.playwright-profiles/{name}-state.json \
  https://example.com
...
**Existing global profiles:**
- `~/.playwright-profiles/claude/` — persistent context (default, use this)

## Session Management

Project-local sessions are stored in `.playwright-sessions/` (gitignored). Use these for task-scoped browser state that doesn't need to persist across projects.
...
### Creating/Using a Project Session
