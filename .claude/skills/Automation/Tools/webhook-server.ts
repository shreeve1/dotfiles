 ...
 * Validates auth, rate-limits, sanitizes input, triggers claude -p, sends Telegram alerts
 */
...
// --- Build claude prompt with injection guard ---
...
function runClaude(prompt: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    execFile(
      ...
      (error, stdout, stderr) => {
        if (error) {
          log(`Claude execution error: ${error.message}`);
          resolve(`Claude analysis failed: ${error.message}`);
          return;
        }
...
async function handleWebhook(
  ...
  // Build prompt and run Claude for investigation
