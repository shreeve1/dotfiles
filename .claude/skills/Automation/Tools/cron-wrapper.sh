...
# Resolve claude binary dynamically
...
if [[ -f "$HOME/.claude/secrets/gog-keyring-password" ]]; then
  export GOG_KEYRING_PASSWORD
  GOG_KEYRING_PASSWORD="$(cat "$HOME/.claude/secrets/gog-keyring-password")"
fi
...
source "$HOME/.claude/secrets/telegram-env.sh" 2>/dev/null || true
...
# --- Lockfile ---
LOCK_DIR="$HOME/.claude/data/locks"
LOCK_PATH=""
if [[ -n "$LOCK_NAME" ]]; then
  mkdir -p "$LOCK_DIR"
...
cleanup() {
  ...
  echo "$LOG_ENTRY" >> "$HOME/.claude/skills/Automation/References/execution-log.jsonl"
  ...
  if [[ $EXIT_CODE -ne 0 ]]; then
    ...
    "$HOME/.claude/skills/Automation/Tools/telegram-send.sh" --silent "$ALERT_MSG" 2>/dev/null || true
  fi
...
# --- Log rotation ---
EXEC_LOG="$HOME/.claude/skills/Automation/References/execution-log.jsonl"
...
if [[ -n "$LOG_FILE" ]]; then
  if [[ "$LOG_FILE" = /* ]]; then
    ...
    ACTUAL_LOG="$HOME/.claude/logs/$LOG_FILE"
  fi
