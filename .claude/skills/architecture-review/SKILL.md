...
1. **Argument is empty:** use the `AskUserQuestion` tool with the following question: "What are we reviewing?", header: "Target", options: [{label: "Single module", description: "one package / project / crate / namespace directory"}, {label: "Single subdirectory", description: "a subtree inside a module"}, {label: "Single file", description: "deep review of one large file"}].
...
1. **Surface the proposal.** Use the `AskUserQuestion` tool with the following question: "Proposed split: {N} top-level layers, {file count} files. L0 — {names or count}; L1 — ...; {sub-layers if any: L3.1, L3.2, ...}. Approve?", header: "Layers", options: [{label: "Approve (Recommended)", description: "accept the proposed split"}, {label: "Adjust split", description: "reorder / merge / split — describe"}, {label: "Reduce scope", description: "drop layers — useful for monolith reviewed one bounded context at a time"}, {label: "Specify manually", description: "specify the split manually"}].

2. **Loop until approved.** Adjustments re-enter Step 2.3 then return here.

### Step 4: Create Skeleton Artifact

1. **Read the template** at `${CLAUDE_SKILL_DIR}/templates/architecture-review.md` FULLY (no limit/offset).
...
1. **Batch (large layer only).** When the layer wasn't pre-decomposed into sub-layers at Step 3 and holds >~10 files, propose batches by file-name cohesion (e.g., all `*Repository.cs`; all `*Configuration.cs`; `DbContext.cs` + migrations; entities). Confirm via the `AskUserQuestion` tool: "Layer {N}: {count} files. Batches: {B1 — count}; {B2 — count}; .... Approve?", header: "Batches", options: [{label: "Approve", description: "accept the proposed batches"}, {label: "Adjust batches", description: "adjust the proposed batches"}, {label: "Promote to sub-layers", description: "each batch becomes L{N}.{x} with its own tally"}].
...
1. **Present each candidate** as a structured triage question using the `AskUserQuestion` tool: "L{X}-{YY} — {headline}. Evidence: `file:line` ({short quote}). Current: {one sentence}. Desired: {one sentence}. Pick a triage outcome.", header: "L{X}-{YY}", options: [{label: "{Concrete option A}", description: "{action}"}, {label: "{Concrete option B}", description: "{action}"}, {label: "Defer", description: "post-release"}].
...
2. **Ask explicitly.** Use the `AskUserQuestion` tool with the following question: "Across {F} findings, did any methodology principle emerge during triage that should be named?", header: "Methodology", options: [{label: "No new principle (Recommended)", description: "if none was articulated — proceed to cross-cut synthesis"}, {label: "Capture one principle", description: "developer describes; skill drafts an M{N} block"}, {label: "Capture multiple", description: "loop one at a time"}].
...
3. **Confirm grouping** via the `AskUserQuestion` tool: "{N} cross-cutting themes: {T1 — name; T2 — name; ...}. Approve grouping or adjust?", header: "Themes", options: [{label: "Approve grouping (Recommended)", description: "accept the proposed grouping"}, {label: "Merge themes", description: "developer names which"}, {label: "Split a theme", description: "developer names which"}].
...
6. **Confirm the plan + flip status.** Use the `AskUserQuestion` tool: "{N} phases ({F} findings across {Files} files). Approve or adjust?", header: "Plan", options: [{label: "Approve (Recommended)", description: "Edit frontmatter status: in-progress → status: ready, proceed to Step 9"}, {label: "Adjust phase boundaries", description: "describe"}, {label: "Resequence phases", description: "describe"}].
...
1. **Display the completion summary:**
   ...
   **Next step (per-phase landing):**
   - `/blueprint .rpiv/artifacts/architecture-reviews/{filename}.md` followed by free-text "Implement Phase 1: {phase name}" — blueprint treats the named phase as the feature scope.
...
1. **Be Structural**: Every checkpoint is `AskUserQuestion` with concrete options. Never prose "what do you think?". The developer answers; the skill captures.
...
- **All checkpoints are `AskUserQuestion`** — no prose "ask the user". The tool always offers free-text via "Other"; don't author free-text prompts.
...
- **Critical ordering:**
  ...
  - ALWAYS triage via `AskUserQuestion` (Step 5.3) — never auto-accept a finding even when the answer seems obvious.
