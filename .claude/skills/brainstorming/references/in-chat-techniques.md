...
- **Facilitator Chosen (default)** — from the goal and the `categories` map, name a batch of 3–4. Confirm exact names with a targeted `list --category` on only the categories you're drawing from; never enumerate the library to choose.
...
- **Inventive Flow** — invent at least 3 techniques, announce the order before the first, touch no script. Log each one's name + description.

The library is large — never pull it whole into context. The only way in is the helper. Subcommands of `uv run {skill-root}/scripts/brain.py --file {skill-root}/assets/brain-methods.csv`:
