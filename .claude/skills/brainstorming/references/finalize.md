...
Record the insights and chosen directions with `uv run {skill-root}/../_shared/scripts/memlog.py append --workspace {doc_workspace} --type insight --text "<insights + chosen directions>"`. **Then run `uv run {skill-root}/../_shared/scripts/memlog.py set --workspace {doc_workspace} --key status --value complete`** — the session is done and must stop being offered for resume. Do this even if the user declines every artifact below.
...
- **Intent doc.** A succinct `brainstorm-intent.md` — the chosen and critical discoveries only, structured to drop straight into a downstream skill (`grill-with-docs`, `to-spec`, `finish-spec`) as clean input, with none of the report's bloat - token usage matters and it must really be on point. Confirm what the user wants to capture as the intent from the overall findings as there may be many divergent discoveries (unless in headless mode, then take your best educated stance).
...
If the session used invented techniques, offer to save a keeper by appending it to `{workflow.additional_techniques}` in `~/.bmad/custom/brainstorming.user.toml` (create the directory and file if missing; show the exact TOML and wait for explicit confirmation before writing).

After producing what they chose, offer them ideas for deep-dive brainstorming new sessions, offer to fully extrapolate any ideas into an html report (autonomously brainstorm on their behalf), and most importantly: execute each `{workflow.external_handoffs}` instruction. Then share the artifact paths (and any handoff destinations), suggest where this leads next — `grill-with-docs` to stress-test the chosen direction (and record the glossary/ADRs it produces in `{project-root}/CONTEXT.md` and `{project-root}/docs/adr/`), then `to-spec` / `finish-spec` / `to-tickets` — and note if a produced intent is detailed enough to hand straight to one of those. Run `{workflow.on_complete}` if non-empty.
