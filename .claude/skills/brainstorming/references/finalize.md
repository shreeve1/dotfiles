...
After producing what they chose, offer them ideas for deep-dive brainstorming new sessions, and offer to fully extrapolate any ideas into an html report (autonomously brainstorm on their behalf). Then share the artifact paths, suggest where this leads next — `grill-with-docs` to stress-test the chosen direction (and record the glossary/ADRs it produces in `{project-root}/CONTEXT.md` and `{project-root}/docs/adr/`), then `to-spec` / `finish-spec` / `to-tickets` — and note if a produced intent is detailed enough to hand straight to one of those.
