...
- **Orca Mac client** (observed 2026-07-26): user `james`, `/Applications/Orca.app`, profile `/Users/james/Library/Application Support/orca`. Bundle version **v1.4.158** (live local bundle, observed 2026-07-26). Client **Active Server** preference set to `Local desktop` on 2026-07-27 (live Orca Settings → Remote Orca Servers → Advanced); this controls the host used by server-routed Add Project, terminals, and provider checks.
- **aidev (Orca runtime host)** (observed 2026-07-25): SSH `james@100.95.230.15`, Ubuntu 24.04 x86_64, private tailnet, fixed port 6768. Binary `/opt/orca/orca-linux.AppImage`, version file `/opt/orca/VERSION`, unit `/etc/systemd/system/orca-serve.service`. Service runs as `james`, HOME `/home/james`, shell `/usr/bin/zsh`. Flags: `--port 6768 --pairing-address 100.95.230.15 --json --mobile-pairing`. Profile `/home/james/.config/orca`. **aidev observed v1.4.158 after a service-manager upgrade on 2026-07-26**. Pi at `/home/james/.npm-global/bin/pi`, herdr at `/home/james/.local/bin/herdr` (observed 2026-07-25).
