...
This workflow walks through 5 phases, interviewing the user at each stage to progressively refine a vague idea into a concrete, buildable PRD. Use the **AskUserQuestion** tool for structured decisions and the **Skill** tool to invoke the `interview` skill for deep-dive questioning.
