...
- If no `PLAN_FILE` is provided, list recent `.md` files in `PLAN_DIRECTORIES` and ask the user to select one using `AskUserQuestion`.
...
3. Present list to user with `AskUserQuestion` showing filename and first line (title)
