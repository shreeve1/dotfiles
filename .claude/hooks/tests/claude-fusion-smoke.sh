...
ONCFG=$(mktemp);  printf '{"claude":"on"}'  > "$ONCFG"
OFFCFG=$(mktemp); printf '{"claude":"off"}' > "$OFFCFG"
# .fusion-off escape-hatch project dir
OFFDIR=$(mktemp -d); mkdir -p "$OFFDIR/.claude"; touch "$OFFDIR/.claude/.fusion-off"
NODIR=/nonexistent
...
for c in '"pi-delegate worker \"do x\""' '"git diff"' '"npm test"' "\"git commit -m 'x'\""; do
