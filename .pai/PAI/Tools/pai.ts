 ...
 *   pai --local          Stay in current directory (don't cd to runtime home)
...
const PAI_DIR = join(process.env.PAI_RUNTIME_HOME || join(homedir(), ".pai"), "PAI");
...
// Note: RAW archiving removed - OpenCode handles its own session retention.
...
async function cmdLaunch(options: { mcp?: string; resume?: boolean; skipPerms?: boolean; local?: boolean }) {
  // OpenCode instructions are loaded from config and startup files.
...
async function cmdUpdate() {
  ...
  const upgradeResult = spawnSync(["opencode", "upgrade"], { stdin: "inherit", stdout: "inherit", stderr: "inherit" });
  if (upgradeResult.exitCode !== 0) error("OpenCode upgrade failed");
...
USAGE:
  ...
  k -l, --local            Stay in current directory (don't cd to runtime home)
