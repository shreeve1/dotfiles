...
function getStats(): SystemStats {
  ...
  try {
    const settings = JSON.parse(readFileSync(join(OPENCODE_DIR, "opencode.json"), "utf-8"));
  ...
  try {
    const latest = readFileSync(join(PAI_DIR, "Algorithm", "LATEST"), "utf-8").trim();
    algorithmVersion = latest.replace(/^v/i, "");
  } catch {}

  try {
    const historyFile = join(OPENCODE_DIR, "history.jsonl");
