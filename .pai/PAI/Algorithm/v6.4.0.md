...
📐 DELEGATION GATE: Subagent delegation is default-ON for non-trivial work. For broad repo/codebase discovery, unfamiliar-code investigation, pattern searches spanning multiple directories, risky review, browser/debug triage, research/current-source work, multi-file edits, or post-change verification, launch the matching `Task` subagent before direct broad tool work. Direct Glob/Grep/Edit is allowed only after delegation starts, or for exact known-file/single-probe work that will not load broad context. If not delegating when a trigger appears to match, write `Delegation exception:` with one narrow reason; valid reasons are: James explicitly asked me to do it myself, no tool work is needed, exact single-file/single-probe task under ~50 lines, or a subagent lacks necessary unstated conversation context.
