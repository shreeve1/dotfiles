...
2. Spawns an `opencode run` session focused on the failing criteria
...
Interactive mode launches a full `opencode run` session with the PRD context pre-loaded. You work with OpenCode directly to make progress on criteria.
...
- Does not send voice notifications during Algorithm entry or phase transitions
