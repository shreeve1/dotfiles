...
**Full voice registry:** `~/.config/opencode/skills/Agents/SKILL.md` (see Named Agents) and `~/.config/opencode/opencode.json` (daidentity.voiceId)
...
| **Telegram** | Bot API | Automation alerts (cron, webhook, PAI routing) | `~/.pai/secrets/telegram-env.sh` |
...
```json
{
  "notifications": {
    ...
    "telegram": {
      "enabled": true,
      "credentialsPath": "~/.pai/secrets/telegram-env.sh"
    },
    "thresholds": {
      "longTaskMinutes": 5
    },
...
1. Bot token and chat ID stored in `~/.pai/secrets/telegram-env.sh` (canonical source)
...
The notification service is in `~/.config/opencode/plugins/lib/notifications.ts`:
...
await sendTelegram("Message");  // reads from ~/.pai/secrets/telegram-env.sh
```
