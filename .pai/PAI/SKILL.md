...
1. **ISC before work.** Create ISC in the active ISA before execution. Depth varies; existence is non-negotiable.
...
- ISA criteria/checklist updates — ISC management
...
|---|-----------|-------------------|-------------|
| 1 | **ISA Criteria** | `ISA.md` criteria and verification tables | ISC creation, tracking, verification |
| 2 | **AskUserQuestion** | Built-in tool | Resolve ambiguity before building the wrong thing |
| 3 | **OpenCode CLI** | `Bash: opencode run "prompt"` | Isolated execution via subprocess |
...
**System of record: ISA.** All ISC live in the active `ISA.md` Criteria and Verification sections. The ISA is the sole source of truth for tracking and proof.
...
- VERIFY → ISA verification evidence and checked ISC rows
- LEARN → CHANGELOG
...
**Thinking-only.** No tool calls except ISA updates, voice curls, context recovery (Grep/Glob/Read, ≤34s).
...
- Populate ideal state and anti-ideal state criteria in the active ISA.
...
- Edge cases → ISA/PRD update.
- Update ISC in the active ISA as needed.
...
- Each criterion: specific evidence → mark ISA criterion passed or failed.
...
- **Completion gate:** reconcile all PASS claims against ISA Verification evidence.
- Update PRD: checkboxes, STATUS, frontmatter.
- Clear stale or contradictory ISC/Verification entries.
...
- **No tool calls in OBSERVE** except ISA updates, voice curls, context recovery.
...
- **No orphaned PASS claims.** Every PASS → ISA Verification evidence.
