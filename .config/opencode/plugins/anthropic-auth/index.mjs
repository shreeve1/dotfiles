...
async function authorize(mode) {
  ...
  url.searchParams.set(
    "redirect_uri",
    "https://claude.ai/oauth/code/callback",
  );
  url.searchParams.set(
    "scope",
...
async function exchange(code, verifier) {
  ...
  const res = await fetch("https://claude.ai/v1/oauth/token", {
    method: "POST",
    headers: authHeaders(),
    body: JSON.stringify({
      code: split[0],
  ...
  if (!res.ok) {
    ...
    console.error(`[anthropic-auth] Token exchange failed: ${res.status} ${res.statusText} - ${errText}`);
    return { type: "failed" };
  }
