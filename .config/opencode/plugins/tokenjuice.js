 ...
 * generated by tokenjuice v0.7.1
 */
...
function clipMiddleWithHashStrict(text, maxChars) {
  if (countTextChars(text) <= maxChars) {
    return { text };
  }
...
function compactWholeJsonText(rawText, maxChars) {
  return parseJsonValue(rawText) === null ? null : clipMiddleWithHashStrict(minifyJsonLexically(rawText), maxChars);
}
...
async function reduceExecutionWithRules(input, rules, opts = {}) {
  ...
  if (inspectionSummary) {
    ...
    const selectedText2 = clampTextMiddleWithMetadata(summaryText, maxInlineChars);
  ...
  if (githubActionsFailureSummary) {
    const inlineText2 = clampTextMiddleWithMetadata(githubActionsFailureSummary.text, maxInlineChars);
