...
# Credentials sourced from ~/.pai/secrets/telegram-env.sh (canonical source)

set -euo pipefail

# Source credentials
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "${PAI_DIR:-$HOME/.pai}/secrets/telegram-env.sh"
...
if [[ -z "${TELEGRAM_BOT_TOKEN:-}" ]]; then
  echo "Error: TELEGRAM_BOT_TOKEN not set (check ~/.pai/secrets/telegram-env.sh)" >&2
  exit 1
fi

if [[ -z "$CHAT_ID" ]]; then
  echo "Error: TELEGRAM_CHAT_ID not set (check ~/.pai/secrets/telegram-env.sh)" >&2
  exit 1
fi
