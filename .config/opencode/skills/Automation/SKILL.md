...
**Use the deterministic generator — never author crontab lines from prose.** Crontab line generation is delegated to `Tools/install-cron.ts` so output is byte-for-byte reproducible across runs. Past AI sessions that authored lines from natural-language templates produced silently-broken entries (positional jobIds the wrapper rejects) — the script removes that failure mode.

To install/update cron entries:

1. Edit `References/cron-jobs.json` (add/remove/modify the job entry).
2. **For LLM/Skill jobs only**: Create a runner script at `~/.pai/scripts/run-JOBNAME.sh` that handles `cd`, env vars, and `exec "${OPENCODE_BIN:-opencode}" run "..."`. Set the entry's `command` to that runner script's path. (Inline `cd /dir && opencode run "..."` does NOT work in crontab — `&&` is interpreted by cron's shell, not the wrapper.)
3. Preview the change:
   ```
   ${PAI_DIR:-$HOME/.pai}/skills/Automation/Tools/install-cron.ts diff
   ```
4. Apply (backs up the current crontab to `~/.pai/data/crontab-backup-<ts>.txt` first):
   ```
   ${PAI_DIR:-$HOME/.pai}/skills/Automation/Tools/install-cron.ts apply
   ```
...
- `--log <path>` — always emitted; absolute path or `~/.pai/logs/<logFile>`.
...
```
*/5 * * * * /home/james/.opencode/skill/Automation/Tools/cron-wrapper.sh --lock itastack-stale-ticket --log /home/james/.pai/logs/itastack-stale-ticket.log --timeout 240 -- /home/james/itastack/scripts/oversight/run-stale-ticket-check.sh
```
...
# a positional argument, not a flag. Past AI sessions produced this form.
0 * * * * /home/james/.opencode/skill/Automation/Tools/cron-wrapper.sh zeroday-supply-chain-monitor
```
...
- Triggers OpenCode for investigation and sends Telegram notifications
...
Credentials are read from `~/.pai/secrets/telegram-env.sh` — the single canonical source.
...
Incoming webhook alerts trigger OpenCode for investigation, then send findings via `telegram-send.sh`.
