---
name: Fabric
description: "Execute 240+ Fabric prompt patterns natively (extraction, summarization, analysis, creation, security, rating). USE WHEN fabric, fabric pattern, run fabric, sync fabric, update patterns, extract_wisdom, threat model, summarize, analyze claims, improve writing, review code, create prd, rate content, mermaid diagram, STRIDE, sigma rules, analyze malware. NOT FOR multi-agent investigation (use Research), dynamic extraction (use ExtractWisdom), threat intel aggregation (use AnnualReports)."
effort: medium
---
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Fabric","workflow":"WORKFLOW_USED","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.pai/PAI/MEMORY/SKILLS/execution.jsonl
```
