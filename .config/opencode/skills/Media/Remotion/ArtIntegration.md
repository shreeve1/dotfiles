...
3. **Use Theme Constants:**
   ```
   ${PAI_DIR:-$HOME/.pai}/skills/Remotion/Tools/Theme.ts
   ```

4. **Reference images** (when visual style reference needed):
   ```
   ${PAI_DIR:-$HOME/.pai}/skills/Media/Art/Examples/
   ```

## PAI Theme Quick Reference

```typescript
import { PAI_THEME } from '${PAI_DIR:-$HOME/.pai}/skills/Remotion/Tools/Theme'
...
```typescript
import { PAI_THEME, titleScreenStyle, fadeInterpolation } from '${PAI_DIR:-$HOME/.pai}/skills/Remotion/Tools/Theme'
