...
const entityIndex = JSON.parse(
  fs.readFileSync('${PAI_DIR:-$HOME/.pai}/skills/parser/entity-index.json', 'utf-8')
);
...
```typescript
import { scrapeAsMarkdown } from '${PAI_DIR:-$HOME/.pai}/skills/mcp/Providers/brightdata/actors';
...
// Main processing
const entityIndexPath = '${PAI_DIR:-$HOME/.pai}/skills/parser/entity-index.json';
...
const schema = JSON.parse(
  fs.readFileSync('${PAI_DIR:-$HOME/.pai}/skills/parser/schema/content-schema.json', 'utf-8')
);
...
processedArticles.forEach((article: any, index: number) => {
  ...
  if (!valid) {
    ...
    const filename = `${PAI_DIR:-$HOME/.pai}/skills/parser/output/${timestamp}_batch-${index + 1}.json`;
...
```bash
# Check entity counts
cat ${PAI_DIR:-$HOME/.pai}/skills/parser/entity-index.json | jq '{
...
URLS=("$@")
SKILL_DIR="$HOME/.opencode/skill/parser"
...
const entityIndexPath = `${process.env.HOME}/.opencode/skill/parser/entity-index.json`;
...
processedArticles.forEach((article: any, index: number) => {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
  const filename = `${process.env.HOME}/.opencode/skill/parser/output/${timestamp}_batch-${index + 1}.json`;
