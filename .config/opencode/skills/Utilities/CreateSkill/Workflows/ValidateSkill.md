...
### Skill Directory
```bash
ls ${PAI_DIR:-$HOME/.pai}/skills/ | grep -i [skillname]
```
...
```bash
ls -la ${PAI_DIR:-$HOME/.pai}/skills/[SkillName]/
```
...
```bash
bun ${PAI_DIR:-$HOME/.pai}/skills/[SkillName]/Tools/[ToolName].ts --help
```
...
```bash
grep -l "Intent-to-Flag" ${PAI_DIR:-$HOME/.pai}/skills/[SkillName]/Workflows/*.md
```
