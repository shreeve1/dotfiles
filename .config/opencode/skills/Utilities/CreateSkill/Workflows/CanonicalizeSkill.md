...
```bash
${PAI_DIR:-$HOME/.pai}/skills/[skill-name]/SKILL.md
```
...
```bash
cp -r ${PAI_DIR:-$HOME/.pai}/skills/[skill-name]/ ~/.pai/History/Backups/[skill-name]-backup-$(date +%Y%m%d)/
```
...
cd ${PAI_DIR:-$HOME/.pai}/skills/[SkillName]/Workflows/
...
find ${PAI_DIR:-$HOME/.pai}/skills/[SkillName]/ -type d -mindepth 2 -maxdepth 3
```
