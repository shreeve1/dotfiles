...
Use `Task(subagent_type="general")` with these specialized agents:
...
```
Task(subagent_type="pai-engineer", isolation: "worktree", prompt="...")
```
...
```
Task(subagent_type="pai-engineer", run_in_background: true, prompt="...")
```
...
bun run ${PAI_DIR:-$HOME/.pai}/skills/Agents/Tools/ComposeAgent.ts --traits "security,skeptical,thorough" --task "Review auth" --output json
...
3. Task(subagent_type="pai-engineer", team_name="my-project", name="auth-engineer")  # Spawn teammate
...
```
Task(subagent_type="general", model="haiku", max_turns=3, prompt="...")
```
...
```
Task(subagent_type="general", prompt="...")  # or specialized agent type
```
