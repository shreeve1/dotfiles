...
- **Task parallelism, not TeamCreate.** UIReviewers are stateless parallel workers. Multiple `Task(subagent_type="ui-reviewer")` calls in one message achieve true parallelism without swarm coordination overhead.
