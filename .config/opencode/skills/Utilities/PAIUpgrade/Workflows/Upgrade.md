...
- List skills in ${PAI_DIR:-$HOME/.pai}/skills/
...
| 2 | Native ${OPENCODE_SESSION_ID} variable | claude-code v2.1.16 | Session IDs are now first-class environment variables everywhere — no more extraction hacks | Session documentation workflows can drop manual ID extraction code |
...
| # | Recommendation | PAI Relevance | Effort | Files Affected |
|---|---------------|---------------|--------|----------------|
| 2 | Replace session ID hacks with native ${OPENCODE_SESSION_ID} | Session documentation workflows have manual extraction workarounds — native variable eliminates fragile code | Low | `skills/_SYSTEM/Workflows/DocumentSession.md` |
...
**What It Is (16-32 words):**
Native environment variable ${OPENCODE_SESSION_ID} is now available in all hooks and commands, eliminating the need for custom session ID extraction or workaround code.
