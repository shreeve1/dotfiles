---
name: goal
description: Set a durable objective the agent works toward across many turns, with a verifiable stopping condition, validation command, and checkpoint-based progress log. Use when the user wants to "follow a goal", "set a goal", "start a goal", "pause/resume/clear a goal", or for code migrations, large refactors, prototype completion, and eval/prompt optimization that have a single concrete end state and a machine-verifiable signal for "done". For a pre-defined task list executed in fresh sessions instead, use `ralph-loop`.
---
