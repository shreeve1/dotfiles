...
metadata:
  source: /Users/james/.pi/agent/skills/pi-merge/SKILL.md
  original_name: pi-merge
---

# Merge and Clean Up

Converted from `/Users/james/.pi/agent/skills/pi-merge/SKILL.md`. Imported as `merge` for OpenCode.
...
If there are uncommitted changes, **commit them automatically** using the `commit` skill (push mode). The feature work should not be lost. Tell the user what was committed.
...
```
question(select): "Merge <branch> into <primary> and clean up?"
Options:
...
| Situation | Action |
|---|---|
| Uncommitted changes | Auto-commit via commit before merge |
...
**Called by:**
- `dev-test` — after tests pass, when user selects merge
- `dev-build` — after build succeeds, when user selects merge

**Invokes:**
- `commit` — to commit any uncommitted work before merge
