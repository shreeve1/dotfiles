...
`~/.pai/PAI/USER/SKILLCUSTOMIZATIONS/Ideate/`
...
Each run persists to `~/.pai/PAI/MEMORY/WORK/{slug}/ideate/`:
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Ideate","workflow":"WORKFLOW_USED","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.pai/PAI/MEMORY/SKILLS/execution.jsonl
```
