...
> **OpenCode fan-out semantics:** This workflow targets OpenCode's Task tool, which fans out *only* when multiple Task calls are issued in a single assistant message — multiple sequential messages with one call each will run serially, not in parallel. Each subagent runs in a fresh isolated context (no access to the launching conversation), so every Task call must include the full argument, the decomposition, the agent's personality block, and the expected return shape. This matches the original PAI design but is worth re-stating because OpenCode does not silently parallelize separate messages.
