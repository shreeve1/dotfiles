 ...
 *
 * Configuration files:
 *   Base:  ${PAI_DIR:-$HOME/.pai}/skills/Agents/Data/Traits.yaml
...
const BASE_TRAITS_PATH = `${HOME}/.opencode/skill/Agents/Data/Traits.yaml`;
const USER_TRAITS_PATH = `${PAI_DIR}/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml`;
const TEMPLATE_PATH = `${HOME}/.opencode/skill/Agents/Templates/DynamicAgent.hbs`;
...
/**
 * Save a composed agent to ~/.pai/custom-agents/{slug}.md
 *
...
\`\`\`bash
bun run ${PAI_DIR:-$HOME/.pai}/skills/Agents/Tools/ComposeAgent.ts --load "${slug}"
\`\`\`

Or reconstruct from traits:

\`\`\`bash
bun run ${PAI_DIR:-$HOME/.pai}/skills/Agents/Tools/ComposeAgent.ts --traits "${agent.traits.join(",")}"
\`\`\`
...
OPTIONS:
  ...
  -s, --save           Save composed agent to ~/.pai/custom-agents/
...
CONFIGURATION:
  Base traits:    ${PAI_DIR:-$HOME/.pai}/skills/Agents/Data/Traits.yaml
  User traits:    ~/.pai/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
  Custom agents:  ~/.pai/custom-agents/
