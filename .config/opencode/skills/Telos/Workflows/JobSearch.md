...
- **TELOS Context**: `~/.pai/PAI/USER/TELOS/`
...
```bash
VENV="${PAI_DIR:-$HOME/.pai}/tools/resume-env"
if [ ! -d "$VENV" ]; then
...
**Fix:** Run the bootstrap command from COMMANDS — it creates the venv and installs weasyprint at `~/.pai/tools/resume-env/`
...
**Fix:** Read the files from `~/.pai/PAI/USER/TELOS/` before proceeding.
