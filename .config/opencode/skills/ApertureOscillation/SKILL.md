...
`~/.pai/PAI/USER/SKILLCUSTOMIZATIONS/ApertureOscillation/`
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"ApertureOscillation","workflow":"Oscillate","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.pai/PAI/MEMORY/SKILLS/execution.jsonl
```
