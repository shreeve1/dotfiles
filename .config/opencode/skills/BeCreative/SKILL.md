...
`~/.pai/PAI/USER/SKILLCUSTOMIZATIONS/BeCreative/`
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"BeCreative","workflow":"WORKFLOW_USED","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.pai/PAI/MEMORY/SKILLS/execution.jsonl
```
