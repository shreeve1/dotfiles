...
`~/.pai/PAI/USER/SKILLCUSTOMIZATIONS/ApiFetcher/`
...
2. When the spec is local, or once a remote spec has been saved locally, use `Bash` to run the helper script:
   ```bash
   python3 ${PAI_DIR:-$HOME/.pai}/skills/ApiFetcher/Tools/openapi_summary.py <spec-file>
   ```
...
The helper script gives you a deterministic inventory of resources and endpoints so you spend less time manually re-parsing large specs. Read `${PAI_DIR:-$HOME/.pai}/skills/ApiFetcher/OpenapiSummarySchema.md` when you need the exact JSON shape or want to map fields into markdown templates.
