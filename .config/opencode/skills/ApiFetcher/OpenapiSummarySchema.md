...
This reference explains the JSON produced by `Tools/openapi_summary.py`.
...
```bash
python3 ${PAI_DIR:-$HOME/.pai}/skills/ApiFetcher/Tools/openapi_summary.py path/to/openapi.yaml
```

The script writes JSON to stdout. Capture it for processing:

```bash
python3 ${PAI_DIR:-$HOME/.pai}/skills/ApiFetcher/Tools/openapi_summary.py path/to/spec.yaml > /tmp/summary.json
```
...
- `name` — parameter name
- `in` — where the parameter appears: `query`, `path`, `header`, or `cookie`
...
3. First concrete path segment (non-parameter)

This makes the output stable enough for automated doc generation while matching common API naming patterns.

## Recommended workflow in the skill

1. Save the spec into `apidocs/source/`
2. Run: `python3 ${PAI_DIR:-$HOME/.pai}/skills/ApiFetcher/Tools/openapi_summary.py apidocs/source/spec.yaml`
