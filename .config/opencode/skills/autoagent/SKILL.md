---
name: autoagent
description: USE WHEN stress test, autoagent, program.md, harness engineering, meta-agent, hill climb, Temporal workflow testing, Temporal schedule experiment, cron experiment, automation testing, score-driven loop, failure mode taxonomy, regression suite, autonomous system improvement. Universal score-driven experiment loop for stress-testing and autonomously improving ANY CLI-driven system under test (SUT) — agent harnesses, Temporal workflows and schedules, cron jobs, CI pipelines, scrapers, scheduled Lambdas. Generalizes kevinrgu/autoagent's hill-climb pattern via a five-part adapter contract (SUT, Mutator, Probe, Verifier, Runner) declared in adapter.yaml, plus a Snapshot block for live systems. Six workflows: CreateProgram (interview → program.md + adapter.yaml), InitAdapter (scaffold adapter for new system type), DesignProbes (Harbor-style experiments targeting failure modes), RunLoop (self-driven baseline → mutate → score → keep/discard with re-entrancy lock + plateau detection), Snapshot (capture/restore live SUT state), AuditSystem (review SUT for stress-testability). Reference adapters ship for autoagent, Temporal, and generic CLI. Produces program.md, adapter.yaml, probes/, snapshots/, results.tsv. NOT for general LLM eval suites (use Evals) or single-prompt testing.
---
