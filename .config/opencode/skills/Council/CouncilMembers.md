...
```bash
bun run ${PAI_DIR:-$HOME/.pai}/skills/Agents/Tools/ComposeAgent.ts \
...
**ALWAYS use `subagent_type: "general"` — NEVER use static types.**
...
|----------|-------|-------|
| Any council debate | `Agent(subagent_type="pai-architect")` | ComposeAgent → `Agent(subagent_type="general")` |
| Security topic | `Agent(subagent_type="general")` | ComposeAgent with `security,adversarial,bold` traits |
| Design question | `Agent(subagent_type="general")` | ComposeAgent with `ux,enthusiastic,exploratory` traits |
| Research needed | `Agent(subagent_type="web-searcher")` | ComposeAgent with `research,thorough,comparative` traits |
