...
Vault root: `~/ob` (resolve `~` from the current user's home directory; do not hardcode `/home/...` or `/Users/...`). Read `~/ob/CLAUDE.md` first — it owns the filing rules.
...
5. Add a one-line entry to `~/ob/Index.md` under the matching section, and append a line to `~/ob/log.md` (`YYYY-MM-DD HH:MM | vault-save | path | summary`).
