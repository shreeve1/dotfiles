...
2. **Confirm via the `AskUserQuestion` tool with the following question:** question "{N} candidates, {D} dimensions. Begin per-candidate fit dispatch?", header "Candidates", options [{label: "Proceed (Recommended)", description: "Begin per-candidate fit dispatch with all {N} candidates and all {D} dimensions"}, {label: "Adjust", description: "Rename, add, or drop candidates; drop dimensions that don't apply"}, {label: "Re-generate", description: "Candidates look wrong — re-run Step 2 with adjusted scope"}].

3. **Handle developer input:**

   **"Proceed"**: lock the candidate × dimension set; advance to Step 4.

   **"Adjust"**: ask the follow-up free-text question with prefix `❓ Question:` — "Which candidates and dimensions should be added, dropped, or renamed?" — apply edits to the working set, re-present, and confirm again with the same three-option `AskUserQuestion`.
...
💬 Follow-up: describe the change in chat to append a timestamped Follow-up section to this artifact. Re-run `/explore` for a fresh artifact.

**Next step:** `/design .rpiv/artifacts/solutions/{filename}.md` — turn the chosen option into a design artifact (or `/blueprint .rpiv/artifacts/solutions/{filename}.md` for the fast path on smaller tasks).
...
- **When to re-invoke instead.** If the candidate set or dimensions shift materially, re-run `/explore` for a fresh artifact. The previous block's `Next step:` stays valid for the existing artifact.
